
obj/kern/kernel：     文件格式 elf32-i386


Disassembly of section .text:

f0100000 <_start+0xeffffff4>:
.globl		_start
_start = RELOC(entry)

.globl entry
entry:
	movw	$0x1234,0x472			# warm boot
f0100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
f0100006:	00 00                	add    %al,(%eax)
f0100008:	fe 4f 52             	decb   0x52(%edi)
f010000b:	e4                   	.byte 0xe4

f010000c <entry>:
f010000c:	66 c7 05 72 04 00 00 	movw   $0x1234,0x472
f0100013:	34 12 
	# sufficient until we set up our real page table in mem_init
	# in lab 2.

	# Load the physical address of entry_pgdir into cr3.  entry_pgdir
	# is defined in entrypgdir.c.
	movl	$(RELOC(entry_pgdir)), %eax
f0100015:	b8 00 00 11 00       	mov    $0x110000,%eax
	movl	%eax, %cr3
f010001a:	0f 22 d8             	mov    %eax,%cr3
	# Turn on paging.
	movl	%cr0, %eax
f010001d:	0f 20 c0             	mov    %cr0,%eax
	orl	$(CR0_PE|CR0_PG|CR0_WP), %eax
f0100020:	0d 01 00 01 80       	or     $0x80010001,%eax
	movl	%eax, %cr0
f0100025:	0f 22 c0             	mov    %eax,%cr0

	# Now paging is enabled, but we're still running at a low EIP
	# (why is this okay?).  Jump up above KERNBASE before entering
	# C code.
	mov	$relocated, %eax
f0100028:	b8 2f 00 10 f0       	mov    $0xf010002f,%eax
	jmp	*%eax
f010002d:	ff e0                	jmp    *%eax

f010002f <relocated>:
relocated:

	# Clear the frame pointer register (EBP)
	# so that once we get into debugging C code,
	# stack backtraces will be terminated properly.
	movl	$0x0,%ebp			# nuke frame pointer
f010002f:	bd 00 00 00 00       	mov    $0x0,%ebp

	# Set the stack pointer
	movl	$(bootstacktop),%esp
f0100034:	bc 00 00 11 f0       	mov    $0xf0110000,%esp

	# now to C code
	call	i386_init
f0100039:	e8 56 00 00 00       	call   f0100094 <i386_init>

f010003e <spin>:

	# Should never get here, but in case we do, just spin.
spin:	jmp	spin
f010003e:	eb fe                	jmp    f010003e <spin>

f0100040 <test_backtrace>:
#include <kern/console.h>

// Test the stack backtrace function (lab 1 only)
void
test_backtrace(int x)
{
f0100040:	55                   	push   %ebp
f0100041:	89 e5                	mov    %esp,%ebp
f0100043:	53                   	push   %ebx
f0100044:	83 ec 0c             	sub    $0xc,%esp
f0100047:	8b 5d 08             	mov    0x8(%ebp),%ebx
	cprintf("entering test_backtrace %d\n", x);
f010004a:	53                   	push   %ebx
f010004b:	68 20 19 10 f0       	push   $0xf0101920
f0100050:	e8 83 09 00 00       	call   f01009d8 <cprintf>
	if (x > 0)
f0100055:	83 c4 10             	add    $0x10,%esp
f0100058:	85 db                	test   %ebx,%ebx
f010005a:	7e 11                	jle    f010006d <test_backtrace+0x2d>
		test_backtrace(x-1);
f010005c:	83 ec 0c             	sub    $0xc,%esp
f010005f:	8d 43 ff             	lea    -0x1(%ebx),%eax
f0100062:	50                   	push   %eax
f0100063:	e8 d8 ff ff ff       	call   f0100040 <test_backtrace>
f0100068:	83 c4 10             	add    $0x10,%esp
f010006b:	eb 11                	jmp    f010007e <test_backtrace+0x3e>
	else
		mon_backtrace(0, 0, 0);
f010006d:	83 ec 04             	sub    $0x4,%esp
f0100070:	6a 00                	push   $0x0
f0100072:	6a 00                	push   $0x0
f0100074:	6a 00                	push   $0x0
f0100076:	e8 0a 07 00 00       	call   f0100785 <mon_backtrace>
f010007b:	83 c4 10             	add    $0x10,%esp
	cprintf("leaving test_backtrace %d\n", x);
f010007e:	83 ec 08             	sub    $0x8,%esp
f0100081:	53                   	push   %ebx
f0100082:	68 3c 19 10 f0       	push   $0xf010193c
f0100087:	e8 4c 09 00 00       	call   f01009d8 <cprintf>
}
f010008c:	83 c4 10             	add    $0x10,%esp
f010008f:	8b 5d fc             	mov    -0x4(%ebp),%ebx
f0100092:	c9                   	leave  
f0100093:	c3                   	ret    

f0100094 <i386_init>:

void
i386_init(void)
{
f0100094:	55                   	push   %ebp
f0100095:	89 e5                	mov    %esp,%ebp
f0100097:	83 ec 0c             	sub    $0xc,%esp
	extern char edata[], end[];

	// Before doing anything else, complete the ELF loading process.
	// Clear the uninitialized global data (BSS) section of our program.
	// This ensures that all static/global variables start out zero.
	memset(edata, 0, end - edata);
f010009a:	b8 44 29 11 f0       	mov    $0xf0112944,%eax
f010009f:	2d 00 23 11 f0       	sub    $0xf0112300,%eax
f01000a4:	50                   	push   %eax
f01000a5:	6a 00                	push   $0x0
f01000a7:	68 00 23 11 f0       	push   $0xf0112300
f01000ac:	e8 d0 13 00 00       	call   f0101481 <memset>

	// Initialize the console.
	// Can't call cprintf until after we do this!
	cons_init();
f01000b1:	e8 9d 04 00 00       	call   f0100553 <cons_init>

	cprintf("6828 decimal is %o octal!\n", 6828);
f01000b6:	83 c4 08             	add    $0x8,%esp
f01000b9:	68 ac 1a 00 00       	push   $0x1aac
f01000be:	68 57 19 10 f0       	push   $0xf0101957
f01000c3:	e8 10 09 00 00       	call   f01009d8 <cprintf>

	// Test the stack backtrace function (lab 1 only)
	test_backtrace(5);
f01000c8:	c7 04 24 05 00 00 00 	movl   $0x5,(%esp)
f01000cf:	e8 6c ff ff ff       	call   f0100040 <test_backtrace>
f01000d4:	83 c4 10             	add    $0x10,%esp

	// Drop into the kernel monitor.
	while (1)
		monitor(NULL);
f01000d7:	83 ec 0c             	sub    $0xc,%esp
f01000da:	6a 00                	push   $0x0
f01000dc:	e8 52 07 00 00       	call   f0100833 <monitor>
f01000e1:	83 c4 10             	add    $0x10,%esp
f01000e4:	eb f1                	jmp    f01000d7 <i386_init+0x43>

f01000e6 <_panic>:
 * Panic is called on unresolvable fatal errors.
 * It prints "panic: mesg", and then enters the kernel monitor.
 */
void
_panic(const char *file, int line, const char *fmt,...)
{
f01000e6:	55                   	push   %ebp
f01000e7:	89 e5                	mov    %esp,%ebp
f01000e9:	56                   	push   %esi
f01000ea:	53                   	push   %ebx
f01000eb:	8b 75 10             	mov    0x10(%ebp),%esi
	va_list ap;

	if (panicstr)
f01000ee:	83 3d 40 29 11 f0 00 	cmpl   $0x0,0xf0112940
f01000f5:	75 37                	jne    f010012e <_panic+0x48>
		goto dead;
	panicstr = fmt;
f01000f7:	89 35 40 29 11 f0    	mov    %esi,0xf0112940

	// Be extra sure that the machine is in as reasonable state
	asm volatile("cli; cld");
f01000fd:	fa                   	cli    
f01000fe:	fc                   	cld    

	va_start(ap, fmt);
f01000ff:	8d 5d 14             	lea    0x14(%ebp),%ebx
	cprintf("kernel panic at %s:%d: ", file, line);
f0100102:	83 ec 04             	sub    $0x4,%esp
f0100105:	ff 75 0c             	pushl  0xc(%ebp)
f0100108:	ff 75 08             	pushl  0x8(%ebp)
f010010b:	68 72 19 10 f0       	push   $0xf0101972
f0100110:	e8 c3 08 00 00       	call   f01009d8 <cprintf>
	vcprintf(fmt, ap);
f0100115:	83 c4 08             	add    $0x8,%esp
f0100118:	53                   	push   %ebx
f0100119:	56                   	push   %esi
f010011a:	e8 93 08 00 00       	call   f01009b2 <vcprintf>
	cprintf("\n");
f010011f:	c7 04 24 ae 19 10 f0 	movl   $0xf01019ae,(%esp)
f0100126:	e8 ad 08 00 00       	call   f01009d8 <cprintf>
	va_end(ap);
f010012b:	83 c4 10             	add    $0x10,%esp

dead:
	/* break into the kernel monitor */
	while (1)
		monitor(NULL);
f010012e:	83 ec 0c             	sub    $0xc,%esp
f0100131:	6a 00                	push   $0x0
f0100133:	e8 fb 06 00 00       	call   f0100833 <monitor>
f0100138:	83 c4 10             	add    $0x10,%esp
f010013b:	eb f1                	jmp    f010012e <_panic+0x48>

f010013d <_warn>:
}

/* like panic, but don't */
void
_warn(const char *file, int line, const char *fmt,...)
{
f010013d:	55                   	push   %ebp
f010013e:	89 e5                	mov    %esp,%ebp
f0100140:	53                   	push   %ebx
f0100141:	83 ec 08             	sub    $0x8,%esp
	va_list ap;

	va_start(ap, fmt);
f0100144:	8d 5d 14             	lea    0x14(%ebp),%ebx
	cprintf("kernel warning at %s:%d: ", file, line);
f0100147:	ff 75 0c             	pushl  0xc(%ebp)
f010014a:	ff 75 08             	pushl  0x8(%ebp)
f010014d:	68 8a 19 10 f0       	push   $0xf010198a
f0100152:	e8 81 08 00 00       	call   f01009d8 <cprintf>
	vcprintf(fmt, ap);
f0100157:	83 c4 08             	add    $0x8,%esp
f010015a:	53                   	push   %ebx
f010015b:	ff 75 10             	pushl  0x10(%ebp)
f010015e:	e8 4f 08 00 00       	call   f01009b2 <vcprintf>
	cprintf("\n");
f0100163:	c7 04 24 ae 19 10 f0 	movl   $0xf01019ae,(%esp)
f010016a:	e8 69 08 00 00       	call   f01009d8 <cprintf>
	va_end(ap);
}
f010016f:	83 c4 10             	add    $0x10,%esp
f0100172:	8b 5d fc             	mov    -0x4(%ebp),%ebx
f0100175:	c9                   	leave  
f0100176:	c3                   	ret    

f0100177 <serial_proc_data>:

static bool serial_exists;

static int
serial_proc_data(void)
{
f0100177:	55                   	push   %ebp
f0100178:	89 e5                	mov    %esp,%ebp

static inline uint8_t
inb(int port)
{
	uint8_t data;
	asm volatile("inb %w1,%0" : "=a" (data) : "d" (port));
f010017a:	ba fd 03 00 00       	mov    $0x3fd,%edx
f010017f:	ec                   	in     (%dx),%al
	if (!(inb(COM1+COM_LSR) & COM_LSR_DATA))
f0100180:	a8 01                	test   $0x1,%al
f0100182:	74 0b                	je     f010018f <serial_proc_data+0x18>
f0100184:	ba f8 03 00 00       	mov    $0x3f8,%edx
f0100189:	ec                   	in     (%dx),%al
		return -1;
	return inb(COM1+COM_RX);
f010018a:	0f b6 c0             	movzbl %al,%eax
f010018d:	eb 05                	jmp    f0100194 <serial_proc_data+0x1d>

static int
serial_proc_data(void)
{
	if (!(inb(COM1+COM_LSR) & COM_LSR_DATA))
		return -1;
f010018f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
	return inb(COM1+COM_RX);
}
f0100194:	5d                   	pop    %ebp
f0100195:	c3                   	ret    

f0100196 <cons_intr>:

// called by device interrupt routines to feed input characters
// into the circular console input buffer.
static void
cons_intr(int (*proc)(void))
{
f0100196:	55                   	push   %ebp
f0100197:	89 e5                	mov    %esp,%ebp
f0100199:	53                   	push   %ebx
f010019a:	83 ec 04             	sub    $0x4,%esp
f010019d:	89 c3                	mov    %eax,%ebx
	int c;

	while ((c = (*proc)()) != -1) {
f010019f:	eb 2b                	jmp    f01001cc <cons_intr+0x36>
		if (c == 0)
f01001a1:	85 c0                	test   %eax,%eax
f01001a3:	74 27                	je     f01001cc <cons_intr+0x36>
			continue;
		cons.buf[cons.wpos++] = c;
f01001a5:	8b 0d 24 25 11 f0    	mov    0xf0112524,%ecx
f01001ab:	8d 51 01             	lea    0x1(%ecx),%edx
f01001ae:	89 15 24 25 11 f0    	mov    %edx,0xf0112524
f01001b4:	88 81 20 23 11 f0    	mov    %al,-0xfeedce0(%ecx)
		if (cons.wpos == CONSBUFSIZE)
f01001ba:	81 fa 00 02 00 00    	cmp    $0x200,%edx
f01001c0:	75 0a                	jne    f01001cc <cons_intr+0x36>
			cons.wpos = 0;
f01001c2:	c7 05 24 25 11 f0 00 	movl   $0x0,0xf0112524
f01001c9:	00 00 00 
static void
cons_intr(int (*proc)(void))
{
	int c;

	while ((c = (*proc)()) != -1) {
f01001cc:	ff d3                	call   *%ebx
f01001ce:	83 f8 ff             	cmp    $0xffffffff,%eax
f01001d1:	75 ce                	jne    f01001a1 <cons_intr+0xb>
			continue;
		cons.buf[cons.wpos++] = c;
		if (cons.wpos == CONSBUFSIZE)
			cons.wpos = 0;
	}
}
f01001d3:	83 c4 04             	add    $0x4,%esp
f01001d6:	5b                   	pop    %ebx
f01001d7:	5d                   	pop    %ebp
f01001d8:	c3                   	ret    

f01001d9 <kbd_proc_data>:
f01001d9:	ba 64 00 00 00       	mov    $0x64,%edx
f01001de:	ec                   	in     (%dx),%al
	int c;
	uint8_t stat, data;
	static uint32_t shift;

	stat = inb(KBSTATP);
	if ((stat & KBS_DIB) == 0)
f01001df:	a8 01                	test   $0x1,%al
f01001e1:	0f 84 f8 00 00 00    	je     f01002df <kbd_proc_data+0x106>
		return -1;
	// Ignore data from mouse.
	if (stat & KBS_TERR)
f01001e7:	a8 20                	test   $0x20,%al
f01001e9:	0f 85 f6 00 00 00    	jne    f01002e5 <kbd_proc_data+0x10c>
f01001ef:	ba 60 00 00 00       	mov    $0x60,%edx
f01001f4:	ec                   	in     (%dx),%al
f01001f5:	89 c2                	mov    %eax,%edx
		return -1;

	data = inb(KBDATAP);

	if (data == 0xE0) {
f01001f7:	3c e0                	cmp    $0xe0,%al
f01001f9:	75 0d                	jne    f0100208 <kbd_proc_data+0x2f>
		// E0 escape character
		shift |= E0ESC;
f01001fb:	83 0d 00 23 11 f0 40 	orl    $0x40,0xf0112300
		return 0;
f0100202:	b8 00 00 00 00       	mov    $0x0,%eax
f0100207:	c3                   	ret    
 * Get data from the keyboard.  If we finish a character, return it.  Else 0.
 * Return -1 if no data.
 */
static int
kbd_proc_data(void)
{
f0100208:	55                   	push   %ebp
f0100209:	89 e5                	mov    %esp,%ebp
f010020b:	53                   	push   %ebx
f010020c:	83 ec 04             	sub    $0x4,%esp

	if (data == 0xE0) {
		// E0 escape character
		shift |= E0ESC;
		return 0;
	} else if (data & 0x80) {
f010020f:	84 c0                	test   %al,%al
f0100211:	79 36                	jns    f0100249 <kbd_proc_data+0x70>
		// Key released
		data = (shift & E0ESC ? data : data & 0x7F);
f0100213:	8b 0d 00 23 11 f0    	mov    0xf0112300,%ecx
f0100219:	89 cb                	mov    %ecx,%ebx
f010021b:	83 e3 40             	and    $0x40,%ebx
f010021e:	83 e0 7f             	and    $0x7f,%eax
f0100221:	85 db                	test   %ebx,%ebx
f0100223:	0f 44 d0             	cmove  %eax,%edx
		shift &= ~(shiftcode[data] | E0ESC);
f0100226:	0f b6 d2             	movzbl %dl,%edx
f0100229:	0f b6 82 00 1b 10 f0 	movzbl -0xfefe500(%edx),%eax
f0100230:	83 c8 40             	or     $0x40,%eax
f0100233:	0f b6 c0             	movzbl %al,%eax
f0100236:	f7 d0                	not    %eax
f0100238:	21 c8                	and    %ecx,%eax
f010023a:	a3 00 23 11 f0       	mov    %eax,0xf0112300
		return 0;
f010023f:	b8 00 00 00 00       	mov    $0x0,%eax
f0100244:	e9 a4 00 00 00       	jmp    f01002ed <kbd_proc_data+0x114>
	} else if (shift & E0ESC) {
f0100249:	8b 0d 00 23 11 f0    	mov    0xf0112300,%ecx
f010024f:	f6 c1 40             	test   $0x40,%cl
f0100252:	74 0e                	je     f0100262 <kbd_proc_data+0x89>
		// Last character was an E0 escape; or with 0x80
		data |= 0x80;
f0100254:	83 c8 80             	or     $0xffffff80,%eax
f0100257:	89 c2                	mov    %eax,%edx
		shift &= ~E0ESC;
f0100259:	83 e1 bf             	and    $0xffffffbf,%ecx
f010025c:	89 0d 00 23 11 f0    	mov    %ecx,0xf0112300
	}

	shift |= shiftcode[data];
f0100262:	0f b6 d2             	movzbl %dl,%edx
	shift ^= togglecode[data];
f0100265:	0f b6 82 00 1b 10 f0 	movzbl -0xfefe500(%edx),%eax
f010026c:	0b 05 00 23 11 f0    	or     0xf0112300,%eax
f0100272:	0f b6 8a 00 1a 10 f0 	movzbl -0xfefe600(%edx),%ecx
f0100279:	31 c8                	xor    %ecx,%eax
f010027b:	a3 00 23 11 f0       	mov    %eax,0xf0112300

	c = charcode[shift & (CTL | SHIFT)][data];
f0100280:	89 c1                	mov    %eax,%ecx
f0100282:	83 e1 03             	and    $0x3,%ecx
f0100285:	8b 0c 8d e0 19 10 f0 	mov    -0xfefe620(,%ecx,4),%ecx
f010028c:	0f b6 14 11          	movzbl (%ecx,%edx,1),%edx
f0100290:	0f b6 da             	movzbl %dl,%ebx
	if (shift & CAPSLOCK) {
f0100293:	a8 08                	test   $0x8,%al
f0100295:	74 1b                	je     f01002b2 <kbd_proc_data+0xd9>
		if ('a' <= c && c <= 'z')
f0100297:	89 da                	mov    %ebx,%edx
f0100299:	8d 4b 9f             	lea    -0x61(%ebx),%ecx
f010029c:	83 f9 19             	cmp    $0x19,%ecx
f010029f:	77 05                	ja     f01002a6 <kbd_proc_data+0xcd>
			c += 'A' - 'a';
f01002a1:	83 eb 20             	sub    $0x20,%ebx
f01002a4:	eb 0c                	jmp    f01002b2 <kbd_proc_data+0xd9>
		else if ('A' <= c && c <= 'Z')
f01002a6:	83 ea 41             	sub    $0x41,%edx
			c += 'a' - 'A';
f01002a9:	8d 4b 20             	lea    0x20(%ebx),%ecx
f01002ac:	83 fa 19             	cmp    $0x19,%edx
f01002af:	0f 46 d9             	cmovbe %ecx,%ebx
	}

	// Process special keys
	// Ctrl-Alt-Del: reboot
	if (!(~shift & (CTL | ALT)) && c == KEY_DEL) {
f01002b2:	f7 d0                	not    %eax
f01002b4:	a8 06                	test   $0x6,%al
f01002b6:	75 33                	jne    f01002eb <kbd_proc_data+0x112>
f01002b8:	81 fb e9 00 00 00    	cmp    $0xe9,%ebx
f01002be:	75 2b                	jne    f01002eb <kbd_proc_data+0x112>
		cprintf("Rebooting!\n");
f01002c0:	83 ec 0c             	sub    $0xc,%esp
f01002c3:	68 a4 19 10 f0       	push   $0xf01019a4
f01002c8:	e8 0b 07 00 00       	call   f01009d8 <cprintf>
}

static inline void
outb(int port, uint8_t data)
{
	asm volatile("outb %0,%w1" : : "a" (data), "d" (port));
f01002cd:	ba 92 00 00 00       	mov    $0x92,%edx
f01002d2:	b8 03 00 00 00       	mov    $0x3,%eax
f01002d7:	ee                   	out    %al,(%dx)
f01002d8:	83 c4 10             	add    $0x10,%esp
		outb(0x92, 0x3); // courtesy of Chris Frost
	}

	return c;
f01002db:	89 d8                	mov    %ebx,%eax
f01002dd:	eb 0e                	jmp    f01002ed <kbd_proc_data+0x114>
	uint8_t stat, data;
	static uint32_t shift;

	stat = inb(KBSTATP);
	if ((stat & KBS_DIB) == 0)
		return -1;
f01002df:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
		cprintf("Rebooting!\n");
		outb(0x92, 0x3); // courtesy of Chris Frost
	}

	return c;
}
f01002e4:	c3                   	ret    
	stat = inb(KBSTATP);
	if ((stat & KBS_DIB) == 0)
		return -1;
	// Ignore data from mouse.
	if (stat & KBS_TERR)
		return -1;
f01002e5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
f01002ea:	c3                   	ret    
	if (!(~shift & (CTL | ALT)) && c == KEY_DEL) {
		cprintf("Rebooting!\n");
		outb(0x92, 0x3); // courtesy of Chris Frost
	}

	return c;
f01002eb:	89 d8                	mov    %ebx,%eax
}
f01002ed:	8b 5d fc             	mov    -0x4(%ebp),%ebx
f01002f0:	c9                   	leave  
f01002f1:	c3                   	ret    

f01002f2 <cons_putc>:
}

// output a character to the console
static void
cons_putc(int c)
{
f01002f2:	55                   	push   %ebp
f01002f3:	89 e5                	mov    %esp,%ebp
f01002f5:	57                   	push   %edi
f01002f6:	56                   	push   %esi
f01002f7:	53                   	push   %ebx
f01002f8:	83 ec 1c             	sub    $0x1c,%esp
f01002fb:	89 c7                	mov    %eax,%edi
static void
serial_putc(int c)
{
	int i;

	for (i = 0;
f01002fd:	bb 00 00 00 00       	mov    $0x0,%ebx

static inline uint8_t
inb(int port)
{
	uint8_t data;
	asm volatile("inb %w1,%0" : "=a" (data) : "d" (port));
f0100302:	be fd 03 00 00       	mov    $0x3fd,%esi
f0100307:	b9 84 00 00 00       	mov    $0x84,%ecx
f010030c:	eb 09                	jmp    f0100317 <cons_putc+0x25>
f010030e:	89 ca                	mov    %ecx,%edx
f0100310:	ec                   	in     (%dx),%al
f0100311:	ec                   	in     (%dx),%al
f0100312:	ec                   	in     (%dx),%al
f0100313:	ec                   	in     (%dx),%al
	     !(inb(COM1 + COM_LSR) & COM_LSR_TXRDY) && i < 12800;
	     i++)
f0100314:	83 c3 01             	add    $0x1,%ebx
f0100317:	89 f2                	mov    %esi,%edx
f0100319:	ec                   	in     (%dx),%al
serial_putc(int c)
{
	int i;

	for (i = 0;
	     !(inb(COM1 + COM_LSR) & COM_LSR_TXRDY) && i < 12800;
f010031a:	a8 20                	test   $0x20,%al
f010031c:	75 08                	jne    f0100326 <cons_putc+0x34>
f010031e:	81 fb ff 31 00 00    	cmp    $0x31ff,%ebx
f0100324:	7e e8                	jle    f010030e <cons_putc+0x1c>
f0100326:	89 f8                	mov    %edi,%eax
f0100328:	88 45 e7             	mov    %al,-0x19(%ebp)
}

static inline void
outb(int port, uint8_t data)
{
	asm volatile("outb %0,%w1" : : "a" (data), "d" (port));
f010032b:	ba f8 03 00 00       	mov    $0x3f8,%edx
f0100330:	ee                   	out    %al,(%dx)
static void
lpt_putc(int c)
{
	int i;

	for (i = 0; !(inb(0x378+1) & 0x80) && i < 12800; i++)
f0100331:	bb 00 00 00 00       	mov    $0x0,%ebx

static inline uint8_t
inb(int port)
{
	uint8_t data;
	asm volatile("inb %w1,%0" : "=a" (data) : "d" (port));
f0100336:	be 79 03 00 00       	mov    $0x379,%esi
f010033b:	b9 84 00 00 00       	mov    $0x84,%ecx
f0100340:	eb 09                	jmp    f010034b <cons_putc+0x59>
f0100342:	89 ca                	mov    %ecx,%edx
f0100344:	ec                   	in     (%dx),%al
f0100345:	ec                   	in     (%dx),%al
f0100346:	ec                   	in     (%dx),%al
f0100347:	ec                   	in     (%dx),%al
f0100348:	83 c3 01             	add    $0x1,%ebx
f010034b:	89 f2                	mov    %esi,%edx
f010034d:	ec                   	in     (%dx),%al
f010034e:	81 fb ff 31 00 00    	cmp    $0x31ff,%ebx
f0100354:	7f 04                	jg     f010035a <cons_putc+0x68>
f0100356:	84 c0                	test   %al,%al
f0100358:	79 e8                	jns    f0100342 <cons_putc+0x50>
}

static inline void
outb(int port, uint8_t data)
{
	asm volatile("outb %0,%w1" : : "a" (data), "d" (port));
f010035a:	ba 78 03 00 00       	mov    $0x378,%edx
f010035f:	0f b6 45 e7          	movzbl -0x19(%ebp),%eax
f0100363:	ee                   	out    %al,(%dx)
f0100364:	ba 7a 03 00 00       	mov    $0x37a,%edx
f0100369:	b8 0d 00 00 00       	mov    $0xd,%eax
f010036e:	ee                   	out    %al,(%dx)
f010036f:	b8 08 00 00 00       	mov    $0x8,%eax
f0100374:	ee                   	out    %al,(%dx)

static void
cga_putc(int c)
{
	// if no attribute given, then use black on white
	if (!(c & ~0xFF))
f0100375:	89 fa                	mov    %edi,%edx
f0100377:	81 e2 00 ff ff ff    	and    $0xffffff00,%edx
		c |= 0x0700;
f010037d:	89 f8                	mov    %edi,%eax
f010037f:	80 cc 07             	or     $0x7,%ah
f0100382:	85 d2                	test   %edx,%edx
f0100384:	0f 44 f8             	cmove  %eax,%edi

	switch (c & 0xff) {
f0100387:	89 f8                	mov    %edi,%eax
f0100389:	0f b6 c0             	movzbl %al,%eax
f010038c:	83 f8 09             	cmp    $0x9,%eax
f010038f:	74 74                	je     f0100405 <cons_putc+0x113>
f0100391:	83 f8 09             	cmp    $0x9,%eax
f0100394:	7f 0a                	jg     f01003a0 <cons_putc+0xae>
f0100396:	83 f8 08             	cmp    $0x8,%eax
f0100399:	74 14                	je     f01003af <cons_putc+0xbd>
f010039b:	e9 99 00 00 00       	jmp    f0100439 <cons_putc+0x147>
f01003a0:	83 f8 0a             	cmp    $0xa,%eax
f01003a3:	74 3a                	je     f01003df <cons_putc+0xed>
f01003a5:	83 f8 0d             	cmp    $0xd,%eax
f01003a8:	74 3d                	je     f01003e7 <cons_putc+0xf5>
f01003aa:	e9 8a 00 00 00       	jmp    f0100439 <cons_putc+0x147>
	case '\b':
		if (crt_pos > 0) {
f01003af:	0f b7 05 28 25 11 f0 	movzwl 0xf0112528,%eax
f01003b6:	66 85 c0             	test   %ax,%ax
f01003b9:	0f 84 e6 00 00 00    	je     f01004a5 <cons_putc+0x1b3>
			crt_pos--;
f01003bf:	83 e8 01             	sub    $0x1,%eax
f01003c2:	66 a3 28 25 11 f0    	mov    %ax,0xf0112528
			crt_buf[crt_pos] = (c & ~0xff) | ' ';
f01003c8:	0f b7 c0             	movzwl %ax,%eax
f01003cb:	66 81 e7 00 ff       	and    $0xff00,%di
f01003d0:	83 cf 20             	or     $0x20,%edi
f01003d3:	8b 15 2c 25 11 f0    	mov    0xf011252c,%edx
f01003d9:	66 89 3c 42          	mov    %di,(%edx,%eax,2)
f01003dd:	eb 78                	jmp    f0100457 <cons_putc+0x165>
		}
		break;
	case '\n':
		crt_pos += CRT_COLS;
f01003df:	66 83 05 28 25 11 f0 	addw   $0x50,0xf0112528
f01003e6:	50 
		/* fallthru */
	case '\r':
		crt_pos -= (crt_pos % CRT_COLS);
f01003e7:	0f b7 05 28 25 11 f0 	movzwl 0xf0112528,%eax
f01003ee:	69 c0 cd cc 00 00    	imul   $0xcccd,%eax,%eax
f01003f4:	c1 e8 16             	shr    $0x16,%eax
f01003f7:	8d 04 80             	lea    (%eax,%eax,4),%eax
f01003fa:	c1 e0 04             	shl    $0x4,%eax
f01003fd:	66 a3 28 25 11 f0    	mov    %ax,0xf0112528
f0100403:	eb 52                	jmp    f0100457 <cons_putc+0x165>
		break;
	case '\t':
		cons_putc(' ');
f0100405:	b8 20 00 00 00       	mov    $0x20,%eax
f010040a:	e8 e3 fe ff ff       	call   f01002f2 <cons_putc>
		cons_putc(' ');
f010040f:	b8 20 00 00 00       	mov    $0x20,%eax
f0100414:	e8 d9 fe ff ff       	call   f01002f2 <cons_putc>
		cons_putc(' ');
f0100419:	b8 20 00 00 00       	mov    $0x20,%eax
f010041e:	e8 cf fe ff ff       	call   f01002f2 <cons_putc>
		cons_putc(' ');
f0100423:	b8 20 00 00 00       	mov    $0x20,%eax
f0100428:	e8 c5 fe ff ff       	call   f01002f2 <cons_putc>
		cons_putc(' ');
f010042d:	b8 20 00 00 00       	mov    $0x20,%eax
f0100432:	e8 bb fe ff ff       	call   f01002f2 <cons_putc>
f0100437:	eb 1e                	jmp    f0100457 <cons_putc+0x165>
		break;
	default:
		crt_buf[crt_pos++] = c;		/* write the character */
f0100439:	0f b7 05 28 25 11 f0 	movzwl 0xf0112528,%eax
f0100440:	8d 50 01             	lea    0x1(%eax),%edx
f0100443:	66 89 15 28 25 11 f0 	mov    %dx,0xf0112528
f010044a:	0f b7 c0             	movzwl %ax,%eax
f010044d:	8b 15 2c 25 11 f0    	mov    0xf011252c,%edx
f0100453:	66 89 3c 42          	mov    %di,(%edx,%eax,2)
		break;
	}

	// What is the purpose of this?
	if (crt_pos >= CRT_SIZE) {
f0100457:	66 81 3d 28 25 11 f0 	cmpw   $0x7cf,0xf0112528
f010045e:	cf 07 
f0100460:	76 43                	jbe    f01004a5 <cons_putc+0x1b3>
		int i;

		memmove(crt_buf, crt_buf + CRT_COLS, (CRT_SIZE - CRT_COLS) * sizeof(uint16_t));
f0100462:	a1 2c 25 11 f0       	mov    0xf011252c,%eax
f0100467:	83 ec 04             	sub    $0x4,%esp
f010046a:	68 00 0f 00 00       	push   $0xf00
f010046f:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
f0100475:	52                   	push   %edx
f0100476:	50                   	push   %eax
f0100477:	e8 52 10 00 00       	call   f01014ce <memmove>
		for (i = CRT_SIZE - CRT_COLS; i < CRT_SIZE; i++)
			crt_buf[i] = 0x0700 | ' ';
f010047c:	8b 15 2c 25 11 f0    	mov    0xf011252c,%edx
f0100482:	8d 82 00 0f 00 00    	lea    0xf00(%edx),%eax
f0100488:	81 c2 a0 0f 00 00    	add    $0xfa0,%edx
f010048e:	83 c4 10             	add    $0x10,%esp
f0100491:	66 c7 00 20 07       	movw   $0x720,(%eax)
f0100496:	83 c0 02             	add    $0x2,%eax
	// What is the purpose of this?
	if (crt_pos >= CRT_SIZE) {
		int i;

		memmove(crt_buf, crt_buf + CRT_COLS, (CRT_SIZE - CRT_COLS) * sizeof(uint16_t));
		for (i = CRT_SIZE - CRT_COLS; i < CRT_SIZE; i++)
f0100499:	39 d0                	cmp    %edx,%eax
f010049b:	75 f4                	jne    f0100491 <cons_putc+0x19f>
			crt_buf[i] = 0x0700 | ' ';
		crt_pos -= CRT_COLS;
f010049d:	66 83 2d 28 25 11 f0 	subw   $0x50,0xf0112528
f01004a4:	50 
	}

	/* move that little blinky thing */
	outb(addr_6845, 14);
f01004a5:	8b 0d 30 25 11 f0    	mov    0xf0112530,%ecx
f01004ab:	b8 0e 00 00 00       	mov    $0xe,%eax
f01004b0:	89 ca                	mov    %ecx,%edx
f01004b2:	ee                   	out    %al,(%dx)
	outb(addr_6845 + 1, crt_pos >> 8);
f01004b3:	0f b7 1d 28 25 11 f0 	movzwl 0xf0112528,%ebx
f01004ba:	8d 71 01             	lea    0x1(%ecx),%esi
f01004bd:	89 d8                	mov    %ebx,%eax
f01004bf:	66 c1 e8 08          	shr    $0x8,%ax
f01004c3:	89 f2                	mov    %esi,%edx
f01004c5:	ee                   	out    %al,(%dx)
f01004c6:	b8 0f 00 00 00       	mov    $0xf,%eax
f01004cb:	89 ca                	mov    %ecx,%edx
f01004cd:	ee                   	out    %al,(%dx)
f01004ce:	89 d8                	mov    %ebx,%eax
f01004d0:	89 f2                	mov    %esi,%edx
f01004d2:	ee                   	out    %al,(%dx)
cons_putc(int c)
{
	serial_putc(c);
	lpt_putc(c);
	cga_putc(c);
}
f01004d3:	8d 65 f4             	lea    -0xc(%ebp),%esp
f01004d6:	5b                   	pop    %ebx
f01004d7:	5e                   	pop    %esi
f01004d8:	5f                   	pop    %edi
f01004d9:	5d                   	pop    %ebp
f01004da:	c3                   	ret    

f01004db <serial_intr>:
}

void
serial_intr(void)
{
	if (serial_exists)
f01004db:	80 3d 34 25 11 f0 00 	cmpb   $0x0,0xf0112534
f01004e2:	74 11                	je     f01004f5 <serial_intr+0x1a>
	return inb(COM1+COM_RX);
}

void
serial_intr(void)
{
f01004e4:	55                   	push   %ebp
f01004e5:	89 e5                	mov    %esp,%ebp
f01004e7:	83 ec 08             	sub    $0x8,%esp
	if (serial_exists)
		cons_intr(serial_proc_data);
f01004ea:	b8 77 01 10 f0       	mov    $0xf0100177,%eax
f01004ef:	e8 a2 fc ff ff       	call   f0100196 <cons_intr>
}
f01004f4:	c9                   	leave  
f01004f5:	f3 c3                	repz ret 

f01004f7 <kbd_intr>:
	return c;
}

void
kbd_intr(void)
{
f01004f7:	55                   	push   %ebp
f01004f8:	89 e5                	mov    %esp,%ebp
f01004fa:	83 ec 08             	sub    $0x8,%esp
	cons_intr(kbd_proc_data);
f01004fd:	b8 d9 01 10 f0       	mov    $0xf01001d9,%eax
f0100502:	e8 8f fc ff ff       	call   f0100196 <cons_intr>
}
f0100507:	c9                   	leave  
f0100508:	c3                   	ret    

f0100509 <cons_getc>:
}

// return the next input character from the console, or 0 if none waiting
int
cons_getc(void)
{
f0100509:	55                   	push   %ebp
f010050a:	89 e5                	mov    %esp,%ebp
f010050c:	83 ec 08             	sub    $0x8,%esp
	int c;

	// poll for any pending input characters,
	// so that this function works even when interrupts are disabled
	// (e.g., when called from the kernel monitor).
	serial_intr();
f010050f:	e8 c7 ff ff ff       	call   f01004db <serial_intr>
	kbd_intr();
f0100514:	e8 de ff ff ff       	call   f01004f7 <kbd_intr>

	// grab the next character from the input buffer.
	if (cons.rpos != cons.wpos) {
f0100519:	a1 20 25 11 f0       	mov    0xf0112520,%eax
f010051e:	3b 05 24 25 11 f0    	cmp    0xf0112524,%eax
f0100524:	74 26                	je     f010054c <cons_getc+0x43>
		c = cons.buf[cons.rpos++];
f0100526:	8d 50 01             	lea    0x1(%eax),%edx
f0100529:	89 15 20 25 11 f0    	mov    %edx,0xf0112520
f010052f:	0f b6 88 20 23 11 f0 	movzbl -0xfeedce0(%eax),%ecx
		if (cons.rpos == CONSBUFSIZE)
			cons.rpos = 0;
		return c;
f0100536:	89 c8                	mov    %ecx,%eax
	kbd_intr();

	// grab the next character from the input buffer.
	if (cons.rpos != cons.wpos) {
		c = cons.buf[cons.rpos++];
		if (cons.rpos == CONSBUFSIZE)
f0100538:	81 fa 00 02 00 00    	cmp    $0x200,%edx
f010053e:	75 11                	jne    f0100551 <cons_getc+0x48>
			cons.rpos = 0;
f0100540:	c7 05 20 25 11 f0 00 	movl   $0x0,0xf0112520
f0100547:	00 00 00 
f010054a:	eb 05                	jmp    f0100551 <cons_getc+0x48>
		return c;
	}
	return 0;
f010054c:	b8 00 00 00 00       	mov    $0x0,%eax
}
f0100551:	c9                   	leave  
f0100552:	c3                   	ret    

f0100553 <cons_init>:
}

// initialize the console devices
void
cons_init(void)
{
f0100553:	55                   	push   %ebp
f0100554:	89 e5                	mov    %esp,%ebp
f0100556:	57                   	push   %edi
f0100557:	56                   	push   %esi
f0100558:	53                   	push   %ebx
f0100559:	83 ec 0c             	sub    $0xc,%esp
	volatile uint16_t *cp;
	uint16_t was;
	unsigned pos;

	cp = (uint16_t*) (KERNBASE + CGA_BUF);
	was = *cp;
f010055c:	0f b7 15 00 80 0b f0 	movzwl 0xf00b8000,%edx
	*cp = (uint16_t) 0xA55A;
f0100563:	66 c7 05 00 80 0b f0 	movw   $0xa55a,0xf00b8000
f010056a:	5a a5 
	if (*cp != 0xA55A) {
f010056c:	0f b7 05 00 80 0b f0 	movzwl 0xf00b8000,%eax
f0100573:	66 3d 5a a5          	cmp    $0xa55a,%ax
f0100577:	74 11                	je     f010058a <cons_init+0x37>
		cp = (uint16_t*) (KERNBASE + MONO_BUF);
		addr_6845 = MONO_BASE;
f0100579:	c7 05 30 25 11 f0 b4 	movl   $0x3b4,0xf0112530
f0100580:	03 00 00 

	cp = (uint16_t*) (KERNBASE + CGA_BUF);
	was = *cp;
	*cp = (uint16_t) 0xA55A;
	if (*cp != 0xA55A) {
		cp = (uint16_t*) (KERNBASE + MONO_BUF);
f0100583:	be 00 00 0b f0       	mov    $0xf00b0000,%esi
f0100588:	eb 16                	jmp    f01005a0 <cons_init+0x4d>
		addr_6845 = MONO_BASE;
	} else {
		*cp = was;
f010058a:	66 89 15 00 80 0b f0 	mov    %dx,0xf00b8000
		addr_6845 = CGA_BASE;
f0100591:	c7 05 30 25 11 f0 d4 	movl   $0x3d4,0xf0112530
f0100598:	03 00 00 
{
	volatile uint16_t *cp;
	uint16_t was;
	unsigned pos;

	cp = (uint16_t*) (KERNBASE + CGA_BUF);
f010059b:	be 00 80 0b f0       	mov    $0xf00b8000,%esi
		*cp = was;
		addr_6845 = CGA_BASE;
	}

	/* Extract cursor location */
	outb(addr_6845, 14);
f01005a0:	8b 3d 30 25 11 f0    	mov    0xf0112530,%edi
f01005a6:	b8 0e 00 00 00       	mov    $0xe,%eax
f01005ab:	89 fa                	mov    %edi,%edx
f01005ad:	ee                   	out    %al,(%dx)
	pos = inb(addr_6845 + 1) << 8;
f01005ae:	8d 5f 01             	lea    0x1(%edi),%ebx

static inline uint8_t
inb(int port)
{
	uint8_t data;
	asm volatile("inb %w1,%0" : "=a" (data) : "d" (port));
f01005b1:	89 da                	mov    %ebx,%edx
f01005b3:	ec                   	in     (%dx),%al
f01005b4:	0f b6 c8             	movzbl %al,%ecx
f01005b7:	c1 e1 08             	shl    $0x8,%ecx
}

static inline void
outb(int port, uint8_t data)
{
	asm volatile("outb %0,%w1" : : "a" (data), "d" (port));
f01005ba:	b8 0f 00 00 00       	mov    $0xf,%eax
f01005bf:	89 fa                	mov    %edi,%edx
f01005c1:	ee                   	out    %al,(%dx)

static inline uint8_t
inb(int port)
{
	uint8_t data;
	asm volatile("inb %w1,%0" : "=a" (data) : "d" (port));
f01005c2:	89 da                	mov    %ebx,%edx
f01005c4:	ec                   	in     (%dx),%al
	outb(addr_6845, 15);
	pos |= inb(addr_6845 + 1);

	crt_buf = (uint16_t*) cp;
f01005c5:	89 35 2c 25 11 f0    	mov    %esi,0xf011252c
	crt_pos = pos;
f01005cb:	0f b6 c0             	movzbl %al,%eax
f01005ce:	09 c8                	or     %ecx,%eax
f01005d0:	66 a3 28 25 11 f0    	mov    %ax,0xf0112528
}

static inline void
outb(int port, uint8_t data)
{
	asm volatile("outb %0,%w1" : : "a" (data), "d" (port));
f01005d6:	be fa 03 00 00       	mov    $0x3fa,%esi
f01005db:	b8 00 00 00 00       	mov    $0x0,%eax
f01005e0:	89 f2                	mov    %esi,%edx
f01005e2:	ee                   	out    %al,(%dx)
f01005e3:	ba fb 03 00 00       	mov    $0x3fb,%edx
f01005e8:	b8 80 ff ff ff       	mov    $0xffffff80,%eax
f01005ed:	ee                   	out    %al,(%dx)
f01005ee:	bb f8 03 00 00       	mov    $0x3f8,%ebx
f01005f3:	b8 0c 00 00 00       	mov    $0xc,%eax
f01005f8:	89 da                	mov    %ebx,%edx
f01005fa:	ee                   	out    %al,(%dx)
f01005fb:	ba f9 03 00 00       	mov    $0x3f9,%edx
f0100600:	b8 00 00 00 00       	mov    $0x0,%eax
f0100605:	ee                   	out    %al,(%dx)
f0100606:	ba fb 03 00 00       	mov    $0x3fb,%edx
f010060b:	b8 03 00 00 00       	mov    $0x3,%eax
f0100610:	ee                   	out    %al,(%dx)
f0100611:	ba fc 03 00 00       	mov    $0x3fc,%edx
f0100616:	b8 00 00 00 00       	mov    $0x0,%eax
f010061b:	ee                   	out    %al,(%dx)
f010061c:	ba f9 03 00 00       	mov    $0x3f9,%edx
f0100621:	b8 01 00 00 00       	mov    $0x1,%eax
f0100626:	ee                   	out    %al,(%dx)

static inline uint8_t
inb(int port)
{
	uint8_t data;
	asm volatile("inb %w1,%0" : "=a" (data) : "d" (port));
f0100627:	ba fd 03 00 00       	mov    $0x3fd,%edx
f010062c:	ec                   	in     (%dx),%al
f010062d:	89 c1                	mov    %eax,%ecx
	// Enable rcv interrupts
	outb(COM1+COM_IER, COM_IER_RDI);

	// Clear any preexisting overrun indications and interrupts
	// Serial port doesn't exist if COM_LSR returns 0xFF
	serial_exists = (inb(COM1+COM_LSR) != 0xFF);
f010062f:	3c ff                	cmp    $0xff,%al
f0100631:	0f 95 05 34 25 11 f0 	setne  0xf0112534
f0100638:	89 f2                	mov    %esi,%edx
f010063a:	ec                   	in     (%dx),%al
f010063b:	89 da                	mov    %ebx,%edx
f010063d:	ec                   	in     (%dx),%al
{
	cga_init();
	kbd_init();
	serial_init();

	if (!serial_exists)
f010063e:	80 f9 ff             	cmp    $0xff,%cl
f0100641:	75 10                	jne    f0100653 <cons_init+0x100>
		cprintf("Serial port does not exist!\n");
f0100643:	83 ec 0c             	sub    $0xc,%esp
f0100646:	68 b0 19 10 f0       	push   $0xf01019b0
f010064b:	e8 88 03 00 00       	call   f01009d8 <cprintf>
f0100650:	83 c4 10             	add    $0x10,%esp
}
f0100653:	8d 65 f4             	lea    -0xc(%ebp),%esp
f0100656:	5b                   	pop    %ebx
f0100657:	5e                   	pop    %esi
f0100658:	5f                   	pop    %edi
f0100659:	5d                   	pop    %ebp
f010065a:	c3                   	ret    

f010065b <cputchar>:

// `High'-level console I/O.  Used by readline and cprintf.

void
cputchar(int c)
{
f010065b:	55                   	push   %ebp
f010065c:	89 e5                	mov    %esp,%ebp
f010065e:	83 ec 08             	sub    $0x8,%esp
	cons_putc(c);
f0100661:	8b 45 08             	mov    0x8(%ebp),%eax
f0100664:	e8 89 fc ff ff       	call   f01002f2 <cons_putc>
}
f0100669:	c9                   	leave  
f010066a:	c3                   	ret    

f010066b <getchar>:

int
getchar(void)
{
f010066b:	55                   	push   %ebp
f010066c:	89 e5                	mov    %esp,%ebp
f010066e:	83 ec 08             	sub    $0x8,%esp
	int c;

	while ((c = cons_getc()) == 0)
f0100671:	e8 93 fe ff ff       	call   f0100509 <cons_getc>
f0100676:	85 c0                	test   %eax,%eax
f0100678:	74 f7                	je     f0100671 <getchar+0x6>
		/* do nothing */;
	return c;
}
f010067a:	c9                   	leave  
f010067b:	c3                   	ret    

f010067c <iscons>:

int
iscons(int fdnum)
{
f010067c:	55                   	push   %ebp
f010067d:	89 e5                	mov    %esp,%ebp
	// used by readline
	return 1;
}
f010067f:	b8 01 00 00 00       	mov    $0x1,%eax
f0100684:	5d                   	pop    %ebp
f0100685:	c3                   	ret    

f0100686 <mon_help>:

/***** Implementations of basic kernel monitor commands *****/

int
mon_help(int argc, char **argv, struct Trapframe *tf)
{
f0100686:	55                   	push   %ebp
f0100687:	89 e5                	mov    %esp,%ebp
f0100689:	83 ec 0c             	sub    $0xc,%esp
	int i;

	for (i = 0; i < ARRAY_SIZE(commands); i++)
		cprintf("%s - %s\n", commands[i].name, commands[i].desc);
f010068c:	68 00 1c 10 f0       	push   $0xf0101c00
f0100691:	68 1e 1c 10 f0       	push   $0xf0101c1e
f0100696:	68 23 1c 10 f0       	push   $0xf0101c23
f010069b:	e8 38 03 00 00       	call   f01009d8 <cprintf>
f01006a0:	83 c4 0c             	add    $0xc,%esp
f01006a3:	68 1c 1d 10 f0       	push   $0xf0101d1c
f01006a8:	68 2c 1c 10 f0       	push   $0xf0101c2c
f01006ad:	68 23 1c 10 f0       	push   $0xf0101c23
f01006b2:	e8 21 03 00 00       	call   f01009d8 <cprintf>
f01006b7:	83 c4 0c             	add    $0xc,%esp
f01006ba:	68 35 1c 10 f0       	push   $0xf0101c35
f01006bf:	68 49 1c 10 f0       	push   $0xf0101c49
f01006c4:	68 23 1c 10 f0       	push   $0xf0101c23
f01006c9:	e8 0a 03 00 00       	call   f01009d8 <cprintf>
	return 0;
}
f01006ce:	b8 00 00 00 00       	mov    $0x0,%eax
f01006d3:	c9                   	leave  
f01006d4:	c3                   	ret    

f01006d5 <mon_kerninfo>:

int
mon_kerninfo(int argc, char **argv, struct Trapframe *tf)
{
f01006d5:	55                   	push   %ebp
f01006d6:	89 e5                	mov    %esp,%ebp
f01006d8:	83 ec 14             	sub    $0x14,%esp
	extern char _start[], entry[], etext[], edata[], end[];

	cprintf("Special kernel symbols:\n");
f01006db:	68 53 1c 10 f0       	push   $0xf0101c53
f01006e0:	e8 f3 02 00 00       	call   f01009d8 <cprintf>
	cprintf("  _start                  %08x (phys)\n", _start);
f01006e5:	83 c4 08             	add    $0x8,%esp
f01006e8:	68 0c 00 10 00       	push   $0x10000c
f01006ed:	68 44 1d 10 f0       	push   $0xf0101d44
f01006f2:	e8 e1 02 00 00       	call   f01009d8 <cprintf>
	cprintf("  entry  %08x (virt)  %08x (phys)\n", entry, entry - KERNBASE);
f01006f7:	83 c4 0c             	add    $0xc,%esp
f01006fa:	68 0c 00 10 00       	push   $0x10000c
f01006ff:	68 0c 00 10 f0       	push   $0xf010000c
f0100704:	68 6c 1d 10 f0       	push   $0xf0101d6c
f0100709:	e8 ca 02 00 00       	call   f01009d8 <cprintf>
	cprintf("  etext  %08x (virt)  %08x (phys)\n", etext, etext - KERNBASE);
f010070e:	83 c4 0c             	add    $0xc,%esp
f0100711:	68 11 19 10 00       	push   $0x101911
f0100716:	68 11 19 10 f0       	push   $0xf0101911
f010071b:	68 90 1d 10 f0       	push   $0xf0101d90
f0100720:	e8 b3 02 00 00       	call   f01009d8 <cprintf>
	cprintf("  edata  %08x (virt)  %08x (phys)\n", edata, edata - KERNBASE);
f0100725:	83 c4 0c             	add    $0xc,%esp
f0100728:	68 00 23 11 00       	push   $0x112300
f010072d:	68 00 23 11 f0       	push   $0xf0112300
f0100732:	68 b4 1d 10 f0       	push   $0xf0101db4
f0100737:	e8 9c 02 00 00       	call   f01009d8 <cprintf>
	cprintf("  end    %08x (virt)  %08x (phys)\n", end, end - KERNBASE);
f010073c:	83 c4 0c             	add    $0xc,%esp
f010073f:	68 44 29 11 00       	push   $0x112944
f0100744:	68 44 29 11 f0       	push   $0xf0112944
f0100749:	68 d8 1d 10 f0       	push   $0xf0101dd8
f010074e:	e8 85 02 00 00       	call   f01009d8 <cprintf>
	cprintf("Kernel executable memory footprint: %dKB\n",
		ROUNDUP(end - entry, 1024) / 1024);
f0100753:	b8 43 2d 11 f0       	mov    $0xf0112d43,%eax
f0100758:	2d 0c 00 10 f0       	sub    $0xf010000c,%eax
	cprintf("  _start                  %08x (phys)\n", _start);
	cprintf("  entry  %08x (virt)  %08x (phys)\n", entry, entry - KERNBASE);
	cprintf("  etext  %08x (virt)  %08x (phys)\n", etext, etext - KERNBASE);
	cprintf("  edata  %08x (virt)  %08x (phys)\n", edata, edata - KERNBASE);
	cprintf("  end    %08x (virt)  %08x (phys)\n", end, end - KERNBASE);
	cprintf("Kernel executable memory footprint: %dKB\n",
f010075d:	83 c4 08             	add    $0x8,%esp
f0100760:	25 00 fc ff ff       	and    $0xfffffc00,%eax
f0100765:	8d 90 ff 03 00 00    	lea    0x3ff(%eax),%edx
f010076b:	85 c0                	test   %eax,%eax
f010076d:	0f 48 c2             	cmovs  %edx,%eax
f0100770:	c1 f8 0a             	sar    $0xa,%eax
f0100773:	50                   	push   %eax
f0100774:	68 fc 1d 10 f0       	push   $0xf0101dfc
f0100779:	e8 5a 02 00 00       	call   f01009d8 <cprintf>
		ROUNDUP(end - entry, 1024) / 1024);
	return 0;
}
f010077e:	b8 00 00 00 00       	mov    $0x0,%eax
f0100783:	c9                   	leave  
f0100784:	c3                   	ret    

f0100785 <mon_backtrace>:

int
mon_backtrace(int argc, char **argv, struct Trapframe *tf)
{
f0100785:	55                   	push   %ebp
f0100786:	89 e5                	mov    %esp,%ebp
f0100788:	57                   	push   %edi
f0100789:	56                   	push   %esi
f010078a:	53                   	push   %ebx
f010078b:	83 ec 38             	sub    $0x38,%esp

static inline uint32_t
read_ebp(void)
{
	uint32_t ebp;
	asm volatile("movl %%ebp,%0" : "=r" (ebp));
f010078e:	89 ee                	mov    %ebp,%esi
	// Your code here.
	unsigned int *ebp = ((unsigned int*)read_ebp());
	cprintf("Stack backtrace:\n");
f0100790:	68 6c 1c 10 f0       	push   $0xf0101c6c
f0100795:	e8 3e 02 00 00       	call   f01009d8 <cprintf>
	while(ebp) {
f010079a:	83 c4 10             	add    $0x10,%esp
f010079d:	eb 7f                	jmp    f010081e <mon_backtrace+0x99>
		cprintf("ebp %08x ", ebp);
f010079f:	83 ec 08             	sub    $0x8,%esp
f01007a2:	56                   	push   %esi
f01007a3:	68 7e 1c 10 f0       	push   $0xf0101c7e
f01007a8:	e8 2b 02 00 00       	call   f01009d8 <cprintf>
		cprintf("eip %08x args", ebp[1]);
f01007ad:	83 c4 08             	add    $0x8,%esp
f01007b0:	ff 76 04             	pushl  0x4(%esi)
f01007b3:	68 88 1c 10 f0       	push   $0xf0101c88
f01007b8:	e8 1b 02 00 00       	call   f01009d8 <cprintf>
f01007bd:	8d 5e 08             	lea    0x8(%esi),%ebx
f01007c0:	8d 7e 1c             	lea    0x1c(%esi),%edi
f01007c3:	83 c4 10             	add    $0x10,%esp
		for(int i = 2; i <= 6; i++)
			cprintf(" %08x", ebp[i]);
f01007c6:	83 ec 08             	sub    $0x8,%esp
f01007c9:	ff 33                	pushl  (%ebx)
f01007cb:	68 96 1c 10 f0       	push   $0xf0101c96
f01007d0:	e8 03 02 00 00       	call   f01009d8 <cprintf>
f01007d5:	83 c3 04             	add    $0x4,%ebx
	unsigned int *ebp = ((unsigned int*)read_ebp());
	cprintf("Stack backtrace:\n");
	while(ebp) {
		cprintf("ebp %08x ", ebp);
		cprintf("eip %08x args", ebp[1]);
		for(int i = 2; i <= 6; i++)
f01007d8:	83 c4 10             	add    $0x10,%esp
f01007db:	39 fb                	cmp    %edi,%ebx
f01007dd:	75 e7                	jne    f01007c6 <mon_backtrace+0x41>
			cprintf(" %08x", ebp[i]);
		cprintf("\n");
f01007df:	83 ec 0c             	sub    $0xc,%esp
f01007e2:	68 ae 19 10 f0       	push   $0xf01019ae
f01007e7:	e8 ec 01 00 00       	call   f01009d8 <cprintf>

		unsigned int eip = ebp[1];
f01007ec:	8b 5e 04             	mov    0x4(%esi),%ebx
		struct Eipdebuginfo info;
		debuginfo_eip(eip, &info);
f01007ef:	83 c4 08             	add    $0x8,%esp
f01007f2:	8d 45 d0             	lea    -0x30(%ebp),%eax
f01007f5:	50                   	push   %eax
f01007f6:	53                   	push   %ebx
f01007f7:	e8 e6 02 00 00       	call   f0100ae2 <debuginfo_eip>
		cprintf("\t%s:%d: %.*s+%d\n",
f01007fc:	83 c4 08             	add    $0x8,%esp
f01007ff:	2b 5d e0             	sub    -0x20(%ebp),%ebx
f0100802:	53                   	push   %ebx
f0100803:	ff 75 d8             	pushl  -0x28(%ebp)
f0100806:	ff 75 dc             	pushl  -0x24(%ebp)
f0100809:	ff 75 d4             	pushl  -0x2c(%ebp)
f010080c:	ff 75 d0             	pushl  -0x30(%ebp)
f010080f:	68 9c 1c 10 f0       	push   $0xf0101c9c
f0100814:	e8 bf 01 00 00       	call   f01009d8 <cprintf>
		info.eip_file, info.eip_line,
		info.eip_fn_namelen, info.eip_fn_name,
		eip-info.eip_fn_addr);

		ebp = (unsigned int*)(*ebp);
f0100819:	8b 36                	mov    (%esi),%esi
f010081b:	83 c4 20             	add    $0x20,%esp
mon_backtrace(int argc, char **argv, struct Trapframe *tf)
{
	// Your code here.
	unsigned int *ebp = ((unsigned int*)read_ebp());
	cprintf("Stack backtrace:\n");
	while(ebp) {
f010081e:	85 f6                	test   %esi,%esi
f0100820:	0f 85 79 ff ff ff    	jne    f010079f <mon_backtrace+0x1a>
		eip-info.eip_fn_addr);

		ebp = (unsigned int*)(*ebp);
	}
	return 0;
}
f0100826:	b8 00 00 00 00       	mov    $0x0,%eax
f010082b:	8d 65 f4             	lea    -0xc(%ebp),%esp
f010082e:	5b                   	pop    %ebx
f010082f:	5e                   	pop    %esi
f0100830:	5f                   	pop    %edi
f0100831:	5d                   	pop    %ebp
f0100832:	c3                   	ret    

f0100833 <monitor>:
	return 0;
}

void
monitor(struct Trapframe *tf)
{
f0100833:	55                   	push   %ebp
f0100834:	89 e5                	mov    %esp,%ebp
f0100836:	57                   	push   %edi
f0100837:	56                   	push   %esi
f0100838:	53                   	push   %ebx
f0100839:	83 ec 68             	sub    $0x68,%esp
	char *buf;

	cprintf("Welcome to the JOS kernel monitor!\n");
f010083c:	68 28 1e 10 f0       	push   $0xf0101e28
f0100841:	e8 92 01 00 00       	call   f01009d8 <cprintf>
	cprintf("Type 'help' for a list of commands.\n");
f0100846:	c7 04 24 4c 1e 10 f0 	movl   $0xf0101e4c,(%esp)
f010084d:	e8 86 01 00 00       	call   f01009d8 <cprintf>
	cprintf("6828 decimal is 15254 octal!\n");
f0100852:	c7 04 24 ad 1c 10 f0 	movl   $0xf0101cad,(%esp)
f0100859:	e8 7a 01 00 00       	call   f01009d8 <cprintf>

   	unsigned int i = 0x00646c72;
f010085e:	c7 45 e4 72 6c 64 00 	movl   $0x646c72,-0x1c(%ebp)
    	cprintf("H%x Wo%s\n", 57616, &i);
f0100865:	83 c4 0c             	add    $0xc,%esp
f0100868:	8d 45 e4             	lea    -0x1c(%ebp),%eax
f010086b:	50                   	push   %eax
f010086c:	68 10 e1 00 00       	push   $0xe110
f0100871:	68 cb 1c 10 f0       	push   $0xf0101ccb
f0100876:	e8 5d 01 00 00       	call   f01009d8 <cprintf>
	cprintf("x=%d y=%d\n", 3);
f010087b:	83 c4 08             	add    $0x8,%esp
f010087e:	6a 03                	push   $0x3
f0100880:	68 d5 1c 10 f0       	push   $0xf0101cd5
f0100885:	e8 4e 01 00 00       	call   f01009d8 <cprintf>
f010088a:	83 c4 10             	add    $0x10,%esp

	while (1) {
		buf = readline("K> ");
f010088d:	83 ec 0c             	sub    $0xc,%esp
f0100890:	68 e0 1c 10 f0       	push   $0xf0101ce0
f0100895:	e8 90 09 00 00       	call   f010122a <readline>
f010089a:	89 c3                	mov    %eax,%ebx
		if (buf != NULL)
f010089c:	83 c4 10             	add    $0x10,%esp
f010089f:	85 c0                	test   %eax,%eax
f01008a1:	74 ea                	je     f010088d <monitor+0x5a>
	char *argv[MAXARGS];
	int i;

	// Parse the command buffer into whitespace-separated arguments
	argc = 0;
	argv[argc] = 0;
f01008a3:	c7 45 a4 00 00 00 00 	movl   $0x0,-0x5c(%ebp)
	int argc;
	char *argv[MAXARGS];
	int i;

	// Parse the command buffer into whitespace-separated arguments
	argc = 0;
f01008aa:	be 00 00 00 00       	mov    $0x0,%esi
f01008af:	eb 0a                	jmp    f01008bb <monitor+0x88>
	argv[argc] = 0;
	while (1) {
		// gobble whitespace
		while (*buf && strchr(WHITESPACE, *buf))
			*buf++ = 0;
f01008b1:	c6 03 00             	movb   $0x0,(%ebx)
f01008b4:	89 f7                	mov    %esi,%edi
f01008b6:	8d 5b 01             	lea    0x1(%ebx),%ebx
f01008b9:	89 fe                	mov    %edi,%esi
	// Parse the command buffer into whitespace-separated arguments
	argc = 0;
	argv[argc] = 0;
	while (1) {
		// gobble whitespace
		while (*buf && strchr(WHITESPACE, *buf))
f01008bb:	0f b6 03             	movzbl (%ebx),%eax
f01008be:	84 c0                	test   %al,%al
f01008c0:	74 63                	je     f0100925 <monitor+0xf2>
f01008c2:	83 ec 08             	sub    $0x8,%esp
f01008c5:	0f be c0             	movsbl %al,%eax
f01008c8:	50                   	push   %eax
f01008c9:	68 e4 1c 10 f0       	push   $0xf0101ce4
f01008ce:	e8 71 0b 00 00       	call   f0101444 <strchr>
f01008d3:	83 c4 10             	add    $0x10,%esp
f01008d6:	85 c0                	test   %eax,%eax
f01008d8:	75 d7                	jne    f01008b1 <monitor+0x7e>
			*buf++ = 0;
		if (*buf == 0)
f01008da:	80 3b 00             	cmpb   $0x0,(%ebx)
f01008dd:	74 46                	je     f0100925 <monitor+0xf2>
			break;

		// save and scan past next arg
		if (argc == MAXARGS-1) {
f01008df:	83 fe 0f             	cmp    $0xf,%esi
f01008e2:	75 14                	jne    f01008f8 <monitor+0xc5>
			cprintf("Too many arguments (max %d)\n", MAXARGS);
f01008e4:	83 ec 08             	sub    $0x8,%esp
f01008e7:	6a 10                	push   $0x10
f01008e9:	68 e9 1c 10 f0       	push   $0xf0101ce9
f01008ee:	e8 e5 00 00 00       	call   f01009d8 <cprintf>
f01008f3:	83 c4 10             	add    $0x10,%esp
f01008f6:	eb 95                	jmp    f010088d <monitor+0x5a>
			return 0;
		}
		argv[argc++] = buf;
f01008f8:	8d 7e 01             	lea    0x1(%esi),%edi
f01008fb:	89 5c b5 a4          	mov    %ebx,-0x5c(%ebp,%esi,4)
f01008ff:	eb 03                	jmp    f0100904 <monitor+0xd1>
		while (*buf && !strchr(WHITESPACE, *buf))
			buf++;
f0100901:	83 c3 01             	add    $0x1,%ebx
		if (argc == MAXARGS-1) {
			cprintf("Too many arguments (max %d)\n", MAXARGS);
			return 0;
		}
		argv[argc++] = buf;
		while (*buf && !strchr(WHITESPACE, *buf))
f0100904:	0f b6 03             	movzbl (%ebx),%eax
f0100907:	84 c0                	test   %al,%al
f0100909:	74 ae                	je     f01008b9 <monitor+0x86>
f010090b:	83 ec 08             	sub    $0x8,%esp
f010090e:	0f be c0             	movsbl %al,%eax
f0100911:	50                   	push   %eax
f0100912:	68 e4 1c 10 f0       	push   $0xf0101ce4
f0100917:	e8 28 0b 00 00       	call   f0101444 <strchr>
f010091c:	83 c4 10             	add    $0x10,%esp
f010091f:	85 c0                	test   %eax,%eax
f0100921:	74 de                	je     f0100901 <monitor+0xce>
f0100923:	eb 94                	jmp    f01008b9 <monitor+0x86>
			buf++;
	}
	argv[argc] = 0;
f0100925:	c7 44 b5 a4 00 00 00 	movl   $0x0,-0x5c(%ebp,%esi,4)
f010092c:	00 

	// Lookup and invoke the command
	if (argc == 0)
f010092d:	85 f6                	test   %esi,%esi
f010092f:	0f 84 58 ff ff ff    	je     f010088d <monitor+0x5a>
f0100935:	bb 00 00 00 00       	mov    $0x0,%ebx
		return 0;
	for (i = 0; i < ARRAY_SIZE(commands); i++) {
		if (strcmp(argv[0], commands[i].name) == 0)
f010093a:	83 ec 08             	sub    $0x8,%esp
f010093d:	8d 04 5b             	lea    (%ebx,%ebx,2),%eax
f0100940:	ff 34 85 80 1e 10 f0 	pushl  -0xfefe180(,%eax,4)
f0100947:	ff 75 a4             	pushl  -0x5c(%ebp)
f010094a:	e8 97 0a 00 00       	call   f01013e6 <strcmp>
f010094f:	83 c4 10             	add    $0x10,%esp
f0100952:	85 c0                	test   %eax,%eax
f0100954:	75 21                	jne    f0100977 <monitor+0x144>
			return commands[i].func(argc, argv, tf);
f0100956:	83 ec 04             	sub    $0x4,%esp
f0100959:	8d 04 5b             	lea    (%ebx,%ebx,2),%eax
f010095c:	ff 75 08             	pushl  0x8(%ebp)
f010095f:	8d 55 a4             	lea    -0x5c(%ebp),%edx
f0100962:	52                   	push   %edx
f0100963:	56                   	push   %esi
f0100964:	ff 14 85 88 1e 10 f0 	call   *-0xfefe178(,%eax,4)
	cprintf("x=%d y=%d\n", 3);

	while (1) {
		buf = readline("K> ");
		if (buf != NULL)
			if (runcmd(buf, tf) < 0)
f010096b:	83 c4 10             	add    $0x10,%esp
f010096e:	85 c0                	test   %eax,%eax
f0100970:	78 25                	js     f0100997 <monitor+0x164>
f0100972:	e9 16 ff ff ff       	jmp    f010088d <monitor+0x5a>
	argv[argc] = 0;

	// Lookup and invoke the command
	if (argc == 0)
		return 0;
	for (i = 0; i < ARRAY_SIZE(commands); i++) {
f0100977:	83 c3 01             	add    $0x1,%ebx
f010097a:	83 fb 03             	cmp    $0x3,%ebx
f010097d:	75 bb                	jne    f010093a <monitor+0x107>
		if (strcmp(argv[0], commands[i].name) == 0)
			return commands[i].func(argc, argv, tf);
	}
	cprintf("Unknown command '%s'\n", argv[0]);
f010097f:	83 ec 08             	sub    $0x8,%esp
f0100982:	ff 75 a4             	pushl  -0x5c(%ebp)
f0100985:	68 06 1d 10 f0       	push   $0xf0101d06
f010098a:	e8 49 00 00 00       	call   f01009d8 <cprintf>
f010098f:	83 c4 10             	add    $0x10,%esp
f0100992:	e9 f6 fe ff ff       	jmp    f010088d <monitor+0x5a>
		buf = readline("K> ");
		if (buf != NULL)
			if (runcmd(buf, tf) < 0)
				break;
	}
}
f0100997:	8d 65 f4             	lea    -0xc(%ebp),%esp
f010099a:	5b                   	pop    %ebx
f010099b:	5e                   	pop    %esi
f010099c:	5f                   	pop    %edi
f010099d:	5d                   	pop    %ebp
f010099e:	c3                   	ret    

f010099f <putch>:
#include <inc/stdarg.h>


static void
putch(int ch, int *cnt)
{
f010099f:	55                   	push   %ebp
f01009a0:	89 e5                	mov    %esp,%ebp
f01009a2:	83 ec 14             	sub    $0x14,%esp
	cputchar(ch);
f01009a5:	ff 75 08             	pushl  0x8(%ebp)
f01009a8:	e8 ae fc ff ff       	call   f010065b <cputchar>
	*cnt++;
}
f01009ad:	83 c4 10             	add    $0x10,%esp
f01009b0:	c9                   	leave  
f01009b1:	c3                   	ret    

f01009b2 <vcprintf>:

int
vcprintf(const char *fmt, va_list ap)
{
f01009b2:	55                   	push   %ebp
f01009b3:	89 e5                	mov    %esp,%ebp
f01009b5:	83 ec 18             	sub    $0x18,%esp
	int cnt = 0;
f01009b8:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)

	vprintfmt((void*)putch, &cnt, fmt, ap);
f01009bf:	ff 75 0c             	pushl  0xc(%ebp)
f01009c2:	ff 75 08             	pushl  0x8(%ebp)
f01009c5:	8d 45 f4             	lea    -0xc(%ebp),%eax
f01009c8:	50                   	push   %eax
f01009c9:	68 9f 09 10 f0       	push   $0xf010099f
f01009ce:	e8 42 04 00 00       	call   f0100e15 <vprintfmt>
	return cnt;
}
f01009d3:	8b 45 f4             	mov    -0xc(%ebp),%eax
f01009d6:	c9                   	leave  
f01009d7:	c3                   	ret    

f01009d8 <cprintf>:

int
cprintf(const char *fmt, ...)
{
f01009d8:	55                   	push   %ebp
f01009d9:	89 e5                	mov    %esp,%ebp
f01009db:	83 ec 10             	sub    $0x10,%esp
	va_list ap;
	int cnt;

	va_start(ap, fmt);
f01009de:	8d 45 0c             	lea    0xc(%ebp),%eax
	cnt = vcprintf(fmt, ap);
f01009e1:	50                   	push   %eax
f01009e2:	ff 75 08             	pushl  0x8(%ebp)
f01009e5:	e8 c8 ff ff ff       	call   f01009b2 <vcprintf>
	va_end(ap);

	return cnt;
}
f01009ea:	c9                   	leave  
f01009eb:	c3                   	ret    

f01009ec <stab_binsearch>:
//	will exit setting left = 118, right = 554.
//
static void
stab_binsearch(const struct Stab *stabs, int *region_left, int *region_right,
	       int type, uintptr_t addr)
{
f01009ec:	55                   	push   %ebp
f01009ed:	89 e5                	mov    %esp,%ebp
f01009ef:	57                   	push   %edi
f01009f0:	56                   	push   %esi
f01009f1:	53                   	push   %ebx
f01009f2:	83 ec 14             	sub    $0x14,%esp
f01009f5:	89 45 ec             	mov    %eax,-0x14(%ebp)
f01009f8:	89 55 e4             	mov    %edx,-0x1c(%ebp)
f01009fb:	89 4d e0             	mov    %ecx,-0x20(%ebp)
f01009fe:	8b 7d 08             	mov    0x8(%ebp),%edi
	int l = *region_left, r = *region_right, any_matches = 0;
f0100a01:	8b 1a                	mov    (%edx),%ebx
f0100a03:	8b 01                	mov    (%ecx),%eax
f0100a05:	89 45 f0             	mov    %eax,-0x10(%ebp)
f0100a08:	c7 45 e8 00 00 00 00 	movl   $0x0,-0x18(%ebp)

	while (l <= r) {
f0100a0f:	eb 7f                	jmp    f0100a90 <stab_binsearch+0xa4>
		int true_m = (l + r) / 2, m = true_m;
f0100a11:	8b 45 f0             	mov    -0x10(%ebp),%eax
f0100a14:	01 d8                	add    %ebx,%eax
f0100a16:	89 c6                	mov    %eax,%esi
f0100a18:	c1 ee 1f             	shr    $0x1f,%esi
f0100a1b:	01 c6                	add    %eax,%esi
f0100a1d:	d1 fe                	sar    %esi
f0100a1f:	8d 04 76             	lea    (%esi,%esi,2),%eax
f0100a22:	8b 4d ec             	mov    -0x14(%ebp),%ecx
f0100a25:	8d 14 81             	lea    (%ecx,%eax,4),%edx
f0100a28:	89 f0                	mov    %esi,%eax

		// search for earliest stab with right type
		while (m >= l && stabs[m].n_type != type)
f0100a2a:	eb 03                	jmp    f0100a2f <stab_binsearch+0x43>
			m--;
f0100a2c:	83 e8 01             	sub    $0x1,%eax

	while (l <= r) {
		int true_m = (l + r) / 2, m = true_m;

		// search for earliest stab with right type
		while (m >= l && stabs[m].n_type != type)
f0100a2f:	39 c3                	cmp    %eax,%ebx
f0100a31:	7f 0d                	jg     f0100a40 <stab_binsearch+0x54>
f0100a33:	0f b6 4a 04          	movzbl 0x4(%edx),%ecx
f0100a37:	83 ea 0c             	sub    $0xc,%edx
f0100a3a:	39 f9                	cmp    %edi,%ecx
f0100a3c:	75 ee                	jne    f0100a2c <stab_binsearch+0x40>
f0100a3e:	eb 05                	jmp    f0100a45 <stab_binsearch+0x59>
			m--;
		if (m < l) {	// no match in [l, m]
			l = true_m + 1;
f0100a40:	8d 5e 01             	lea    0x1(%esi),%ebx
			continue;
f0100a43:	eb 4b                	jmp    f0100a90 <stab_binsearch+0xa4>
		}

		// actual binary search
		any_matches = 1;
		if (stabs[m].n_value < addr) {
f0100a45:	8d 14 40             	lea    (%eax,%eax,2),%edx
f0100a48:	8b 4d ec             	mov    -0x14(%ebp),%ecx
f0100a4b:	8b 54 91 08          	mov    0x8(%ecx,%edx,4),%edx
f0100a4f:	39 55 0c             	cmp    %edx,0xc(%ebp)
f0100a52:	76 11                	jbe    f0100a65 <stab_binsearch+0x79>
			*region_left = m;
f0100a54:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
f0100a57:	89 03                	mov    %eax,(%ebx)
			l = true_m + 1;
f0100a59:	8d 5e 01             	lea    0x1(%esi),%ebx
			l = true_m + 1;
			continue;
		}

		// actual binary search
		any_matches = 1;
f0100a5c:	c7 45 e8 01 00 00 00 	movl   $0x1,-0x18(%ebp)
f0100a63:	eb 2b                	jmp    f0100a90 <stab_binsearch+0xa4>
		if (stabs[m].n_value < addr) {
			*region_left = m;
			l = true_m + 1;
		} else if (stabs[m].n_value > addr) {
f0100a65:	39 55 0c             	cmp    %edx,0xc(%ebp)
f0100a68:	73 14                	jae    f0100a7e <stab_binsearch+0x92>
			*region_right = m - 1;
f0100a6a:	83 e8 01             	sub    $0x1,%eax
f0100a6d:	89 45 f0             	mov    %eax,-0x10(%ebp)
f0100a70:	8b 75 e0             	mov    -0x20(%ebp),%esi
f0100a73:	89 06                	mov    %eax,(%esi)
			l = true_m + 1;
			continue;
		}

		// actual binary search
		any_matches = 1;
f0100a75:	c7 45 e8 01 00 00 00 	movl   $0x1,-0x18(%ebp)
f0100a7c:	eb 12                	jmp    f0100a90 <stab_binsearch+0xa4>
			*region_right = m - 1;
			r = m - 1;
		} else {
			// exact match for 'addr', but continue loop to find
			// *region_right
			*region_left = m;
f0100a7e:	8b 75 e4             	mov    -0x1c(%ebp),%esi
f0100a81:	89 06                	mov    %eax,(%esi)
			l = m;
			addr++;
f0100a83:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
f0100a87:	89 c3                	mov    %eax,%ebx
			l = true_m + 1;
			continue;
		}

		// actual binary search
		any_matches = 1;
f0100a89:	c7 45 e8 01 00 00 00 	movl   $0x1,-0x18(%ebp)
stab_binsearch(const struct Stab *stabs, int *region_left, int *region_right,
	       int type, uintptr_t addr)
{
	int l = *region_left, r = *region_right, any_matches = 0;

	while (l <= r) {
f0100a90:	3b 5d f0             	cmp    -0x10(%ebp),%ebx
f0100a93:	0f 8e 78 ff ff ff    	jle    f0100a11 <stab_binsearch+0x25>
			l = m;
			addr++;
		}
	}

	if (!any_matches)
f0100a99:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
f0100a9d:	75 0f                	jne    f0100aae <stab_binsearch+0xc2>
		*region_right = *region_left - 1;
f0100a9f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
f0100aa2:	8b 00                	mov    (%eax),%eax
f0100aa4:	83 e8 01             	sub    $0x1,%eax
f0100aa7:	8b 75 e0             	mov    -0x20(%ebp),%esi
f0100aaa:	89 06                	mov    %eax,(%esi)
f0100aac:	eb 2c                	jmp    f0100ada <stab_binsearch+0xee>
	else {
		// find rightmost region containing 'addr'
		for (l = *region_right;
f0100aae:	8b 45 e0             	mov    -0x20(%ebp),%eax
f0100ab1:	8b 00                	mov    (%eax),%eax
		     l > *region_left && stabs[l].n_type != type;
f0100ab3:	8b 75 e4             	mov    -0x1c(%ebp),%esi
f0100ab6:	8b 0e                	mov    (%esi),%ecx
f0100ab8:	8d 14 40             	lea    (%eax,%eax,2),%edx
f0100abb:	8b 75 ec             	mov    -0x14(%ebp),%esi
f0100abe:	8d 14 96             	lea    (%esi,%edx,4),%edx

	if (!any_matches)
		*region_right = *region_left - 1;
	else {
		// find rightmost region containing 'addr'
		for (l = *region_right;
f0100ac1:	eb 03                	jmp    f0100ac6 <stab_binsearch+0xda>
		     l > *region_left && stabs[l].n_type != type;
		     l--)
f0100ac3:	83 e8 01             	sub    $0x1,%eax

	if (!any_matches)
		*region_right = *region_left - 1;
	else {
		// find rightmost region containing 'addr'
		for (l = *region_right;
f0100ac6:	39 c8                	cmp    %ecx,%eax
f0100ac8:	7e 0b                	jle    f0100ad5 <stab_binsearch+0xe9>
		     l > *region_left && stabs[l].n_type != type;
f0100aca:	0f b6 5a 04          	movzbl 0x4(%edx),%ebx
f0100ace:	83 ea 0c             	sub    $0xc,%edx
f0100ad1:	39 df                	cmp    %ebx,%edi
f0100ad3:	75 ee                	jne    f0100ac3 <stab_binsearch+0xd7>
		     l--)
			/* do nothing */;
		*region_left = l;
f0100ad5:	8b 75 e4             	mov    -0x1c(%ebp),%esi
f0100ad8:	89 06                	mov    %eax,(%esi)
	}
}
f0100ada:	83 c4 14             	add    $0x14,%esp
f0100add:	5b                   	pop    %ebx
f0100ade:	5e                   	pop    %esi
f0100adf:	5f                   	pop    %edi
f0100ae0:	5d                   	pop    %ebp
f0100ae1:	c3                   	ret    

f0100ae2 <debuginfo_eip>:
//	negative if not.  But even if it returns negative it has stored some
//	information into '*info'.
//
int
debuginfo_eip(uintptr_t addr, struct Eipdebuginfo *info)
{
f0100ae2:	55                   	push   %ebp
f0100ae3:	89 e5                	mov    %esp,%ebp
f0100ae5:	57                   	push   %edi
f0100ae6:	56                   	push   %esi
f0100ae7:	53                   	push   %ebx
f0100ae8:	83 ec 3c             	sub    $0x3c,%esp
f0100aeb:	8b 75 08             	mov    0x8(%ebp),%esi
f0100aee:	8b 5d 0c             	mov    0xc(%ebp),%ebx
	const struct Stab *stabs, *stab_end;
	const char *stabstr, *stabstr_end;
	int lfile, rfile, lfun, rfun, lline, rline;

	// Initialize *info
	info->eip_file = "<unknown>";
f0100af1:	c7 03 a4 1e 10 f0    	movl   $0xf0101ea4,(%ebx)
	info->eip_line = 0;
f0100af7:	c7 43 04 00 00 00 00 	movl   $0x0,0x4(%ebx)
	info->eip_fn_name = "<unknown>";
f0100afe:	c7 43 08 a4 1e 10 f0 	movl   $0xf0101ea4,0x8(%ebx)
	info->eip_fn_namelen = 9;
f0100b05:	c7 43 0c 09 00 00 00 	movl   $0x9,0xc(%ebx)
	info->eip_fn_addr = addr;
f0100b0c:	89 73 10             	mov    %esi,0x10(%ebx)
	info->eip_fn_narg = 0;
f0100b0f:	c7 43 14 00 00 00 00 	movl   $0x0,0x14(%ebx)

	// Find the relevant set of stabs
	if (addr >= ULIM) {
f0100b16:	81 fe ff ff 7f ef    	cmp    $0xef7fffff,%esi
f0100b1c:	76 11                	jbe    f0100b2f <debuginfo_eip+0x4d>
		// Can't search for user-level addresses yet!
  	        panic("User address");
	}

	// String table validity checks
	if (stabstr_end <= stabstr || stabstr_end[-1] != 0)
f0100b1e:	b8 0a 74 10 f0       	mov    $0xf010740a,%eax
f0100b23:	3d d9 5a 10 f0       	cmp    $0xf0105ad9,%eax
f0100b28:	77 19                	ja     f0100b43 <debuginfo_eip+0x61>
f0100b2a:	e9 a1 01 00 00       	jmp    f0100cd0 <debuginfo_eip+0x1ee>
		stab_end = __STAB_END__;
		stabstr = __STABSTR_BEGIN__;
		stabstr_end = __STABSTR_END__;
	} else {
		// Can't search for user-level addresses yet!
  	        panic("User address");
f0100b2f:	83 ec 04             	sub    $0x4,%esp
f0100b32:	68 ae 1e 10 f0       	push   $0xf0101eae
f0100b37:	6a 7f                	push   $0x7f
f0100b39:	68 bb 1e 10 f0       	push   $0xf0101ebb
f0100b3e:	e8 a3 f5 ff ff       	call   f01000e6 <_panic>
	}

	// String table validity checks
	if (stabstr_end <= stabstr || stabstr_end[-1] != 0)
f0100b43:	80 3d 09 74 10 f0 00 	cmpb   $0x0,0xf0107409
f0100b4a:	0f 85 87 01 00 00    	jne    f0100cd7 <debuginfo_eip+0x1f5>
	// 'eip'.  First, we find the basic source file containing 'eip'.
	// Then, we look in that source file for the function.  Then we look
	// for the line number.

	// Search the entire set of stabs for the source file (type N_SO).
	lfile = 0;
f0100b50:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
	rfile = (stab_end - stabs) - 1;
f0100b57:	b8 d8 5a 10 f0       	mov    $0xf0105ad8,%eax
f0100b5c:	2d dc 20 10 f0       	sub    $0xf01020dc,%eax
f0100b61:	c1 f8 02             	sar    $0x2,%eax
f0100b64:	69 c0 ab aa aa aa    	imul   $0xaaaaaaab,%eax,%eax
f0100b6a:	83 e8 01             	sub    $0x1,%eax
f0100b6d:	89 45 e0             	mov    %eax,-0x20(%ebp)
	stab_binsearch(stabs, &lfile, &rfile, N_SO, addr);
f0100b70:	83 ec 08             	sub    $0x8,%esp
f0100b73:	56                   	push   %esi
f0100b74:	6a 64                	push   $0x64
f0100b76:	8d 4d e0             	lea    -0x20(%ebp),%ecx
f0100b79:	8d 55 e4             	lea    -0x1c(%ebp),%edx
f0100b7c:	b8 dc 20 10 f0       	mov    $0xf01020dc,%eax
f0100b81:	e8 66 fe ff ff       	call   f01009ec <stab_binsearch>
	if (lfile == 0)
f0100b86:	8b 45 e4             	mov    -0x1c(%ebp),%eax
f0100b89:	83 c4 10             	add    $0x10,%esp
f0100b8c:	85 c0                	test   %eax,%eax
f0100b8e:	0f 84 4a 01 00 00    	je     f0100cde <debuginfo_eip+0x1fc>
		return -1;

	// Search within that file's stabs for the function definition
	// (N_FUN).
	lfun = lfile;
f0100b94:	89 45 dc             	mov    %eax,-0x24(%ebp)
	rfun = rfile;
f0100b97:	8b 45 e0             	mov    -0x20(%ebp),%eax
f0100b9a:	89 45 d8             	mov    %eax,-0x28(%ebp)
	stab_binsearch(stabs, &lfun, &rfun, N_FUN, addr);
f0100b9d:	83 ec 08             	sub    $0x8,%esp
f0100ba0:	56                   	push   %esi
f0100ba1:	6a 24                	push   $0x24
f0100ba3:	8d 4d d8             	lea    -0x28(%ebp),%ecx
f0100ba6:	8d 55 dc             	lea    -0x24(%ebp),%edx
f0100ba9:	b8 dc 20 10 f0       	mov    $0xf01020dc,%eax
f0100bae:	e8 39 fe ff ff       	call   f01009ec <stab_binsearch>

	if (lfun <= rfun) {
f0100bb3:	8b 45 dc             	mov    -0x24(%ebp),%eax
f0100bb6:	8b 55 d8             	mov    -0x28(%ebp),%edx
f0100bb9:	83 c4 10             	add    $0x10,%esp
f0100bbc:	39 d0                	cmp    %edx,%eax
f0100bbe:	7f 40                	jg     f0100c00 <debuginfo_eip+0x11e>
		// stabs[lfun] points to the function name
		// in the string table, but check bounds just in case.
		if (stabs[lfun].n_strx < stabstr_end - stabstr)
f0100bc0:	8d 0c 40             	lea    (%eax,%eax,2),%ecx
f0100bc3:	c1 e1 02             	shl    $0x2,%ecx
f0100bc6:	8d b9 dc 20 10 f0    	lea    -0xfefdf24(%ecx),%edi
f0100bcc:	89 7d c4             	mov    %edi,-0x3c(%ebp)
f0100bcf:	8b b9 dc 20 10 f0    	mov    -0xfefdf24(%ecx),%edi
f0100bd5:	b9 0a 74 10 f0       	mov    $0xf010740a,%ecx
f0100bda:	81 e9 d9 5a 10 f0    	sub    $0xf0105ad9,%ecx
f0100be0:	39 cf                	cmp    %ecx,%edi
f0100be2:	73 09                	jae    f0100bed <debuginfo_eip+0x10b>
			info->eip_fn_name = stabstr + stabs[lfun].n_strx;
f0100be4:	81 c7 d9 5a 10 f0    	add    $0xf0105ad9,%edi
f0100bea:	89 7b 08             	mov    %edi,0x8(%ebx)
		info->eip_fn_addr = stabs[lfun].n_value;
f0100bed:	8b 7d c4             	mov    -0x3c(%ebp),%edi
f0100bf0:	8b 4f 08             	mov    0x8(%edi),%ecx
f0100bf3:	89 4b 10             	mov    %ecx,0x10(%ebx)
		addr -= info->eip_fn_addr;
f0100bf6:	29 ce                	sub    %ecx,%esi
		// Search within the function definition for the line number.
		lline = lfun;
f0100bf8:	89 45 d4             	mov    %eax,-0x2c(%ebp)
		rline = rfun;
f0100bfb:	89 55 d0             	mov    %edx,-0x30(%ebp)
f0100bfe:	eb 0f                	jmp    f0100c0f <debuginfo_eip+0x12d>
	} else {
		// Couldn't find function stab!  Maybe we're in an assembly
		// file.  Search the whole file for the line number.
		info->eip_fn_addr = addr;
f0100c00:	89 73 10             	mov    %esi,0x10(%ebx)
		lline = lfile;
f0100c03:	8b 45 e4             	mov    -0x1c(%ebp),%eax
f0100c06:	89 45 d4             	mov    %eax,-0x2c(%ebp)
		rline = rfile;
f0100c09:	8b 45 e0             	mov    -0x20(%ebp),%eax
f0100c0c:	89 45 d0             	mov    %eax,-0x30(%ebp)
	}
	// Ignore stuff after the colon.
	info->eip_fn_namelen = strfind(info->eip_fn_name, ':') - info->eip_fn_name;
f0100c0f:	83 ec 08             	sub    $0x8,%esp
f0100c12:	6a 3a                	push   $0x3a
f0100c14:	ff 73 08             	pushl  0x8(%ebx)
f0100c17:	e8 49 08 00 00       	call   f0101465 <strfind>
f0100c1c:	2b 43 08             	sub    0x8(%ebx),%eax
f0100c1f:	89 43 0c             	mov    %eax,0xc(%ebx)
	// Hint:
	//	There's a particular stabs type used for line numbers.
	//	Look at the STABS documentation and <inc/stab.h> to find
	//	which one.
	// Your code here.
	stab_binsearch(stabs, &lline, &rline, N_SLINE, addr);
f0100c22:	83 c4 08             	add    $0x8,%esp
f0100c25:	56                   	push   %esi
f0100c26:	6a 44                	push   $0x44
f0100c28:	8d 4d d0             	lea    -0x30(%ebp),%ecx
f0100c2b:	8d 55 d4             	lea    -0x2c(%ebp),%edx
f0100c2e:	b8 dc 20 10 f0       	mov    $0xf01020dc,%eax
f0100c33:	e8 b4 fd ff ff       	call   f01009ec <stab_binsearch>
	    info->eip_line = stabs[lline].n_desc;
f0100c38:	8b 55 d4             	mov    -0x2c(%ebp),%edx
f0100c3b:	8d 04 52             	lea    (%edx,%edx,2),%eax
f0100c3e:	8d 04 85 dc 20 10 f0 	lea    -0xfefdf24(,%eax,4),%eax
f0100c45:	0f b7 48 06          	movzwl 0x6(%eax),%ecx
f0100c49:	89 4b 04             	mov    %ecx,0x4(%ebx)
	// Search backwards from the line number for the relevant filename
	// stab.
	// We can't just use the "lfile" stab because inlined functions
	// can interpolate code from a different file!
	// Such included source files use the N_SOL stab type.
	while (lline >= lfile
f0100c4c:	8b 75 e4             	mov    -0x1c(%ebp),%esi
f0100c4f:	83 c4 10             	add    $0x10,%esp
f0100c52:	eb 06                	jmp    f0100c5a <debuginfo_eip+0x178>
f0100c54:	83 ea 01             	sub    $0x1,%edx
f0100c57:	83 e8 0c             	sub    $0xc,%eax
f0100c5a:	39 d6                	cmp    %edx,%esi
f0100c5c:	7f 34                	jg     f0100c92 <debuginfo_eip+0x1b0>
	       && stabs[lline].n_type != N_SOL
f0100c5e:	0f b6 48 04          	movzbl 0x4(%eax),%ecx
f0100c62:	80 f9 84             	cmp    $0x84,%cl
f0100c65:	74 0b                	je     f0100c72 <debuginfo_eip+0x190>
	       && (stabs[lline].n_type != N_SO || !stabs[lline].n_value))
f0100c67:	80 f9 64             	cmp    $0x64,%cl
f0100c6a:	75 e8                	jne    f0100c54 <debuginfo_eip+0x172>
f0100c6c:	83 78 08 00          	cmpl   $0x0,0x8(%eax)
f0100c70:	74 e2                	je     f0100c54 <debuginfo_eip+0x172>
		lline--;
	if (lline >= lfile && stabs[lline].n_strx < stabstr_end - stabstr)
f0100c72:	8d 04 52             	lea    (%edx,%edx,2),%eax
f0100c75:	8b 14 85 dc 20 10 f0 	mov    -0xfefdf24(,%eax,4),%edx
f0100c7c:	b8 0a 74 10 f0       	mov    $0xf010740a,%eax
f0100c81:	2d d9 5a 10 f0       	sub    $0xf0105ad9,%eax
f0100c86:	39 c2                	cmp    %eax,%edx
f0100c88:	73 08                	jae    f0100c92 <debuginfo_eip+0x1b0>
		info->eip_file = stabstr + stabs[lline].n_strx;
f0100c8a:	81 c2 d9 5a 10 f0    	add    $0xf0105ad9,%edx
f0100c90:	89 13                	mov    %edx,(%ebx)


	// Set eip_fn_narg to the number of arguments taken by the function,
	// or 0 if there was no containing function.
	if (lfun < rfun)
f0100c92:	8b 55 dc             	mov    -0x24(%ebp),%edx
f0100c95:	8b 75 d8             	mov    -0x28(%ebp),%esi
		for (lline = lfun + 1;
		     lline < rfun && stabs[lline].n_type == N_PSYM;
		     lline++)
			info->eip_fn_narg++;

	return 0;
f0100c98:	b8 00 00 00 00       	mov    $0x0,%eax
		info->eip_file = stabstr + stabs[lline].n_strx;


	// Set eip_fn_narg to the number of arguments taken by the function,
	// or 0 if there was no containing function.
	if (lfun < rfun)
f0100c9d:	39 f2                	cmp    %esi,%edx
f0100c9f:	7d 49                	jge    f0100cea <debuginfo_eip+0x208>
		for (lline = lfun + 1;
f0100ca1:	83 c2 01             	add    $0x1,%edx
f0100ca4:	89 d0                	mov    %edx,%eax
f0100ca6:	8d 14 52             	lea    (%edx,%edx,2),%edx
f0100ca9:	8d 14 95 dc 20 10 f0 	lea    -0xfefdf24(,%edx,4),%edx
f0100cb0:	eb 04                	jmp    f0100cb6 <debuginfo_eip+0x1d4>
		     lline < rfun && stabs[lline].n_type == N_PSYM;
		     lline++)
			info->eip_fn_narg++;
f0100cb2:	83 43 14 01          	addl   $0x1,0x14(%ebx)


	// Set eip_fn_narg to the number of arguments taken by the function,
	// or 0 if there was no containing function.
	if (lfun < rfun)
		for (lline = lfun + 1;
f0100cb6:	39 c6                	cmp    %eax,%esi
f0100cb8:	7e 2b                	jle    f0100ce5 <debuginfo_eip+0x203>
		     lline < rfun && stabs[lline].n_type == N_PSYM;
f0100cba:	0f b6 4a 04          	movzbl 0x4(%edx),%ecx
f0100cbe:	83 c0 01             	add    $0x1,%eax
f0100cc1:	83 c2 0c             	add    $0xc,%edx
f0100cc4:	80 f9 a0             	cmp    $0xa0,%cl
f0100cc7:	74 e9                	je     f0100cb2 <debuginfo_eip+0x1d0>
		     lline++)
			info->eip_fn_narg++;

	return 0;
f0100cc9:	b8 00 00 00 00       	mov    $0x0,%eax
f0100cce:	eb 1a                	jmp    f0100cea <debuginfo_eip+0x208>
  	        panic("User address");
	}

	// String table validity checks
	if (stabstr_end <= stabstr || stabstr_end[-1] != 0)
		return -1;
f0100cd0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
f0100cd5:	eb 13                	jmp    f0100cea <debuginfo_eip+0x208>
f0100cd7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
f0100cdc:	eb 0c                	jmp    f0100cea <debuginfo_eip+0x208>
	// Search the entire set of stabs for the source file (type N_SO).
	lfile = 0;
	rfile = (stab_end - stabs) - 1;
	stab_binsearch(stabs, &lfile, &rfile, N_SO, addr);
	if (lfile == 0)
		return -1;
f0100cde:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
f0100ce3:	eb 05                	jmp    f0100cea <debuginfo_eip+0x208>
		for (lline = lfun + 1;
		     lline < rfun && stabs[lline].n_type == N_PSYM;
		     lline++)
			info->eip_fn_narg++;

	return 0;
f0100ce5:	b8 00 00 00 00       	mov    $0x0,%eax
}
f0100cea:	8d 65 f4             	lea    -0xc(%ebp),%esp
f0100ced:	5b                   	pop    %ebx
f0100cee:	5e                   	pop    %esi
f0100cef:	5f                   	pop    %edi
f0100cf0:	5d                   	pop    %ebp
f0100cf1:	c3                   	ret    

f0100cf2 <printnum>:
 * using specified putch function and associated pointer putdat.
 */
static void
printnum(void (*putch)(int, void*), void *putdat,
	 unsigned long long num, unsigned base, int width, int padc)
{
f0100cf2:	55                   	push   %ebp
f0100cf3:	89 e5                	mov    %esp,%ebp
f0100cf5:	57                   	push   %edi
f0100cf6:	56                   	push   %esi
f0100cf7:	53                   	push   %ebx
f0100cf8:	83 ec 1c             	sub    $0x1c,%esp
f0100cfb:	89 c7                	mov    %eax,%edi
f0100cfd:	89 d6                	mov    %edx,%esi
f0100cff:	8b 45 08             	mov    0x8(%ebp),%eax
f0100d02:	8b 55 0c             	mov    0xc(%ebp),%edx
f0100d05:	89 45 d8             	mov    %eax,-0x28(%ebp)
f0100d08:	89 55 dc             	mov    %edx,-0x24(%ebp)
	// first recursively print all preceding (more significant) digits
	if (num >= base) {
f0100d0b:	8b 4d 10             	mov    0x10(%ebp),%ecx
f0100d0e:	bb 00 00 00 00       	mov    $0x0,%ebx
f0100d13:	89 4d e0             	mov    %ecx,-0x20(%ebp)
f0100d16:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
f0100d19:	39 d3                	cmp    %edx,%ebx
f0100d1b:	72 05                	jb     f0100d22 <printnum+0x30>
f0100d1d:	39 45 10             	cmp    %eax,0x10(%ebp)
f0100d20:	77 45                	ja     f0100d67 <printnum+0x75>
		printnum(putch, putdat, num / base, base, width - 1, padc);
f0100d22:	83 ec 0c             	sub    $0xc,%esp
f0100d25:	ff 75 18             	pushl  0x18(%ebp)
f0100d28:	8b 45 14             	mov    0x14(%ebp),%eax
f0100d2b:	8d 58 ff             	lea    -0x1(%eax),%ebx
f0100d2e:	53                   	push   %ebx
f0100d2f:	ff 75 10             	pushl  0x10(%ebp)
f0100d32:	83 ec 08             	sub    $0x8,%esp
f0100d35:	ff 75 e4             	pushl  -0x1c(%ebp)
f0100d38:	ff 75 e0             	pushl  -0x20(%ebp)
f0100d3b:	ff 75 dc             	pushl  -0x24(%ebp)
f0100d3e:	ff 75 d8             	pushl  -0x28(%ebp)
f0100d41:	e8 4a 09 00 00       	call   f0101690 <__udivdi3>
f0100d46:	83 c4 18             	add    $0x18,%esp
f0100d49:	52                   	push   %edx
f0100d4a:	50                   	push   %eax
f0100d4b:	89 f2                	mov    %esi,%edx
f0100d4d:	89 f8                	mov    %edi,%eax
f0100d4f:	e8 9e ff ff ff       	call   f0100cf2 <printnum>
f0100d54:	83 c4 20             	add    $0x20,%esp
f0100d57:	eb 18                	jmp    f0100d71 <printnum+0x7f>
	} else {
		// print any needed pad characters before first digit
		while (--width > 0)
			putch(padc, putdat);
f0100d59:	83 ec 08             	sub    $0x8,%esp
f0100d5c:	56                   	push   %esi
f0100d5d:	ff 75 18             	pushl  0x18(%ebp)
f0100d60:	ff d7                	call   *%edi
f0100d62:	83 c4 10             	add    $0x10,%esp
f0100d65:	eb 03                	jmp    f0100d6a <printnum+0x78>
f0100d67:	8b 5d 14             	mov    0x14(%ebp),%ebx
	// first recursively print all preceding (more significant) digits
	if (num >= base) {
		printnum(putch, putdat, num / base, base, width - 1, padc);
	} else {
		// print any needed pad characters before first digit
		while (--width > 0)
f0100d6a:	83 eb 01             	sub    $0x1,%ebx
f0100d6d:	85 db                	test   %ebx,%ebx
f0100d6f:	7f e8                	jg     f0100d59 <printnum+0x67>
			putch(padc, putdat);
	}

	// then print this (the least significant) digit
	putch("0123456789abcdef"[num % base], putdat);
f0100d71:	83 ec 08             	sub    $0x8,%esp
f0100d74:	56                   	push   %esi
f0100d75:	83 ec 04             	sub    $0x4,%esp
f0100d78:	ff 75 e4             	pushl  -0x1c(%ebp)
f0100d7b:	ff 75 e0             	pushl  -0x20(%ebp)
f0100d7e:	ff 75 dc             	pushl  -0x24(%ebp)
f0100d81:	ff 75 d8             	pushl  -0x28(%ebp)
f0100d84:	e8 37 0a 00 00       	call   f01017c0 <__umoddi3>
f0100d89:	83 c4 14             	add    $0x14,%esp
f0100d8c:	0f be 80 c9 1e 10 f0 	movsbl -0xfefe137(%eax),%eax
f0100d93:	50                   	push   %eax
f0100d94:	ff d7                	call   *%edi
}
f0100d96:	83 c4 10             	add    $0x10,%esp
f0100d99:	8d 65 f4             	lea    -0xc(%ebp),%esp
f0100d9c:	5b                   	pop    %ebx
f0100d9d:	5e                   	pop    %esi
f0100d9e:	5f                   	pop    %edi
f0100d9f:	5d                   	pop    %ebp
f0100da0:	c3                   	ret    

f0100da1 <getuint>:

// Get an unsigned int of various possible sizes from a varargs list,
// depending on the lflag parameter.
static unsigned long long
getuint(va_list *ap, int lflag)
{
f0100da1:	55                   	push   %ebp
f0100da2:	89 e5                	mov    %esp,%ebp
	if (lflag >= 2)
f0100da4:	83 fa 01             	cmp    $0x1,%edx
f0100da7:	7e 0e                	jle    f0100db7 <getuint+0x16>
		return va_arg(*ap, unsigned long long);
f0100da9:	8b 10                	mov    (%eax),%edx
f0100dab:	8d 4a 08             	lea    0x8(%edx),%ecx
f0100dae:	89 08                	mov    %ecx,(%eax)
f0100db0:	8b 02                	mov    (%edx),%eax
f0100db2:	8b 52 04             	mov    0x4(%edx),%edx
f0100db5:	eb 22                	jmp    f0100dd9 <getuint+0x38>
	else if (lflag)
f0100db7:	85 d2                	test   %edx,%edx
f0100db9:	74 10                	je     f0100dcb <getuint+0x2a>
		return va_arg(*ap, unsigned long);
f0100dbb:	8b 10                	mov    (%eax),%edx
f0100dbd:	8d 4a 04             	lea    0x4(%edx),%ecx
f0100dc0:	89 08                	mov    %ecx,(%eax)
f0100dc2:	8b 02                	mov    (%edx),%eax
f0100dc4:	ba 00 00 00 00       	mov    $0x0,%edx
f0100dc9:	eb 0e                	jmp    f0100dd9 <getuint+0x38>
	else
		return va_arg(*ap, unsigned int);
f0100dcb:	8b 10                	mov    (%eax),%edx
f0100dcd:	8d 4a 04             	lea    0x4(%edx),%ecx
f0100dd0:	89 08                	mov    %ecx,(%eax)
f0100dd2:	8b 02                	mov    (%edx),%eax
f0100dd4:	ba 00 00 00 00       	mov    $0x0,%edx
}
f0100dd9:	5d                   	pop    %ebp
f0100dda:	c3                   	ret    

f0100ddb <sprintputch>:
	int cnt;
};

static void
sprintputch(int ch, struct sprintbuf *b)
{
f0100ddb:	55                   	push   %ebp
f0100ddc:	89 e5                	mov    %esp,%ebp
f0100dde:	8b 45 0c             	mov    0xc(%ebp),%eax
	b->cnt++;
f0100de1:	83 40 08 01          	addl   $0x1,0x8(%eax)
	if (b->buf < b->ebuf)
f0100de5:	8b 10                	mov    (%eax),%edx
f0100de7:	3b 50 04             	cmp    0x4(%eax),%edx
f0100dea:	73 0a                	jae    f0100df6 <sprintputch+0x1b>
		*b->buf++ = ch;
f0100dec:	8d 4a 01             	lea    0x1(%edx),%ecx
f0100def:	89 08                	mov    %ecx,(%eax)
f0100df1:	8b 45 08             	mov    0x8(%ebp),%eax
f0100df4:	88 02                	mov    %al,(%edx)
}
f0100df6:	5d                   	pop    %ebp
f0100df7:	c3                   	ret    

f0100df8 <printfmt>:
	}
}

void
printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...)
{
f0100df8:	55                   	push   %ebp
f0100df9:	89 e5                	mov    %esp,%ebp
f0100dfb:	83 ec 08             	sub    $0x8,%esp
	va_list ap;

	va_start(ap, fmt);
f0100dfe:	8d 45 14             	lea    0x14(%ebp),%eax
	vprintfmt(putch, putdat, fmt, ap);
f0100e01:	50                   	push   %eax
f0100e02:	ff 75 10             	pushl  0x10(%ebp)
f0100e05:	ff 75 0c             	pushl  0xc(%ebp)
f0100e08:	ff 75 08             	pushl  0x8(%ebp)
f0100e0b:	e8 05 00 00 00       	call   f0100e15 <vprintfmt>
	va_end(ap);
}
f0100e10:	83 c4 10             	add    $0x10,%esp
f0100e13:	c9                   	leave  
f0100e14:	c3                   	ret    

f0100e15 <vprintfmt>:
// Main function to format and print a string.
void printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...);

void
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap)
{
f0100e15:	55                   	push   %ebp
f0100e16:	89 e5                	mov    %esp,%ebp
f0100e18:	57                   	push   %edi
f0100e19:	56                   	push   %esi
f0100e1a:	53                   	push   %ebx
f0100e1b:	83 ec 2c             	sub    $0x2c,%esp
f0100e1e:	8b 75 08             	mov    0x8(%ebp),%esi
f0100e21:	8b 5d 0c             	mov    0xc(%ebp),%ebx
f0100e24:	8b 7d 10             	mov    0x10(%ebp),%edi
f0100e27:	eb 12                	jmp    f0100e3b <vprintfmt+0x26>
	int base, lflag, width, precision, altflag;
	char padc;

	while (1) {
		while ((ch = *(unsigned char *) fmt++) != '%') {
			if (ch == '\0')
f0100e29:	85 c0                	test   %eax,%eax
f0100e2b:	0f 84 89 03 00 00    	je     f01011ba <vprintfmt+0x3a5>
				return;
			putch(ch, putdat);
f0100e31:	83 ec 08             	sub    $0x8,%esp
f0100e34:	53                   	push   %ebx
f0100e35:	50                   	push   %eax
f0100e36:	ff d6                	call   *%esi
f0100e38:	83 c4 10             	add    $0x10,%esp
	unsigned long long num;
	int base, lflag, width, precision, altflag;
	char padc;

	while (1) {
		while ((ch = *(unsigned char *) fmt++) != '%') {
f0100e3b:	83 c7 01             	add    $0x1,%edi
f0100e3e:	0f b6 47 ff          	movzbl -0x1(%edi),%eax
f0100e42:	83 f8 25             	cmp    $0x25,%eax
f0100e45:	75 e2                	jne    f0100e29 <vprintfmt+0x14>
f0100e47:	c6 45 d4 20          	movb   $0x20,-0x2c(%ebp)
f0100e4b:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)
f0100e52:	c7 45 d0 ff ff ff ff 	movl   $0xffffffff,-0x30(%ebp)
f0100e59:	c7 45 e0 ff ff ff ff 	movl   $0xffffffff,-0x20(%ebp)
f0100e60:	ba 00 00 00 00       	mov    $0x0,%edx
f0100e65:	eb 07                	jmp    f0100e6e <vprintfmt+0x59>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100e67:	8b 7d e4             	mov    -0x1c(%ebp),%edi

		// flag to pad on the right
		case '-':
			padc = '-';
f0100e6a:	c6 45 d4 2d          	movb   $0x2d,-0x2c(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100e6e:	8d 47 01             	lea    0x1(%edi),%eax
f0100e71:	89 45 e4             	mov    %eax,-0x1c(%ebp)
f0100e74:	0f b6 07             	movzbl (%edi),%eax
f0100e77:	0f b6 c8             	movzbl %al,%ecx
f0100e7a:	83 e8 23             	sub    $0x23,%eax
f0100e7d:	3c 55                	cmp    $0x55,%al
f0100e7f:	0f 87 1a 03 00 00    	ja     f010119f <vprintfmt+0x38a>
f0100e85:	0f b6 c0             	movzbl %al,%eax
f0100e88:	ff 24 85 58 1f 10 f0 	jmp    *-0xfefe0a8(,%eax,4)
f0100e8f:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			padc = '-';
			goto reswitch;

		// flag to pad with 0's instead of spaces
		case '0':
			padc = '0';
f0100e92:	c6 45 d4 30          	movb   $0x30,-0x2c(%ebp)
f0100e96:	eb d6                	jmp    f0100e6e <vprintfmt+0x59>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100e98:	8b 7d e4             	mov    -0x1c(%ebp),%edi
f0100e9b:	b8 00 00 00 00       	mov    $0x0,%eax
f0100ea0:	89 55 e4             	mov    %edx,-0x1c(%ebp)
		case '6':
		case '7':
		case '8':
		case '9':
			for (precision = 0; ; ++fmt) {
				precision = precision * 10 + ch - '0';
f0100ea3:	8d 04 80             	lea    (%eax,%eax,4),%eax
f0100ea6:	8d 44 41 d0          	lea    -0x30(%ecx,%eax,2),%eax
				ch = *fmt;
f0100eaa:	0f be 0f             	movsbl (%edi),%ecx
				if (ch < '0' || ch > '9')
f0100ead:	8d 51 d0             	lea    -0x30(%ecx),%edx
f0100eb0:	83 fa 09             	cmp    $0x9,%edx
f0100eb3:	77 39                	ja     f0100eee <vprintfmt+0xd9>
		case '5':
		case '6':
		case '7':
		case '8':
		case '9':
			for (precision = 0; ; ++fmt) {
f0100eb5:	83 c7 01             	add    $0x1,%edi
				precision = precision * 10 + ch - '0';
				ch = *fmt;
				if (ch < '0' || ch > '9')
					break;
			}
f0100eb8:	eb e9                	jmp    f0100ea3 <vprintfmt+0x8e>
			goto process_precision;

		case '*':
			precision = va_arg(ap, int);
f0100eba:	8b 45 14             	mov    0x14(%ebp),%eax
f0100ebd:	8d 48 04             	lea    0x4(%eax),%ecx
f0100ec0:	89 4d 14             	mov    %ecx,0x14(%ebp)
f0100ec3:	8b 00                	mov    (%eax),%eax
f0100ec5:	89 45 d0             	mov    %eax,-0x30(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100ec8:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			}
			goto process_precision;

		case '*':
			precision = va_arg(ap, int);
			goto process_precision;
f0100ecb:	eb 27                	jmp    f0100ef4 <vprintfmt+0xdf>
f0100ecd:	8b 45 e0             	mov    -0x20(%ebp),%eax
f0100ed0:	85 c0                	test   %eax,%eax
f0100ed2:	b9 00 00 00 00       	mov    $0x0,%ecx
f0100ed7:	0f 49 c8             	cmovns %eax,%ecx
f0100eda:	89 4d e0             	mov    %ecx,-0x20(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100edd:	8b 7d e4             	mov    -0x1c(%ebp),%edi
f0100ee0:	eb 8c                	jmp    f0100e6e <vprintfmt+0x59>
f0100ee2:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			if (width < 0)
				width = 0;
			goto reswitch;

		case '#':
			altflag = 1;
f0100ee5:	c7 45 d8 01 00 00 00 	movl   $0x1,-0x28(%ebp)
			goto reswitch;
f0100eec:	eb 80                	jmp    f0100e6e <vprintfmt+0x59>
f0100eee:	8b 55 e4             	mov    -0x1c(%ebp),%edx
f0100ef1:	89 45 d0             	mov    %eax,-0x30(%ebp)

		process_precision:
			if (width < 0)
f0100ef4:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
f0100ef8:	0f 89 70 ff ff ff    	jns    f0100e6e <vprintfmt+0x59>
				width = precision, precision = -1;
f0100efe:	8b 45 d0             	mov    -0x30(%ebp),%eax
f0100f01:	89 45 e0             	mov    %eax,-0x20(%ebp)
f0100f04:	c7 45 d0 ff ff ff ff 	movl   $0xffffffff,-0x30(%ebp)
f0100f0b:	e9 5e ff ff ff       	jmp    f0100e6e <vprintfmt+0x59>
			goto reswitch;

		// long flag (doubled for long long)
		case 'l':
			lflag++;
f0100f10:	83 c2 01             	add    $0x1,%edx
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100f13:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			goto reswitch;

		// long flag (doubled for long long)
		case 'l':
			lflag++;
			goto reswitch;
f0100f16:	e9 53 ff ff ff       	jmp    f0100e6e <vprintfmt+0x59>

		// character
		case 'c':
			putch(va_arg(ap, int), putdat);
f0100f1b:	8b 45 14             	mov    0x14(%ebp),%eax
f0100f1e:	8d 50 04             	lea    0x4(%eax),%edx
f0100f21:	89 55 14             	mov    %edx,0x14(%ebp)
f0100f24:	83 ec 08             	sub    $0x8,%esp
f0100f27:	53                   	push   %ebx
f0100f28:	ff 30                	pushl  (%eax)
f0100f2a:	ff d6                	call   *%esi
			break;
f0100f2c:	83 c4 10             	add    $0x10,%esp
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100f2f:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			goto reswitch;

		// character
		case 'c':
			putch(va_arg(ap, int), putdat);
			break;
f0100f32:	e9 04 ff ff ff       	jmp    f0100e3b <vprintfmt+0x26>

		// error message
		case 'e':
			err = va_arg(ap, int);
f0100f37:	8b 45 14             	mov    0x14(%ebp),%eax
f0100f3a:	8d 50 04             	lea    0x4(%eax),%edx
f0100f3d:	89 55 14             	mov    %edx,0x14(%ebp)
f0100f40:	8b 00                	mov    (%eax),%eax
f0100f42:	99                   	cltd   
f0100f43:	31 d0                	xor    %edx,%eax
f0100f45:	29 d0                	sub    %edx,%eax
			if (err < 0)
				err = -err;
			if (err >= MAXERROR || (p = error_string[err]) == NULL)
f0100f47:	83 f8 06             	cmp    $0x6,%eax
f0100f4a:	7f 0b                	jg     f0100f57 <vprintfmt+0x142>
f0100f4c:	8b 14 85 b0 20 10 f0 	mov    -0xfefdf50(,%eax,4),%edx
f0100f53:	85 d2                	test   %edx,%edx
f0100f55:	75 18                	jne    f0100f6f <vprintfmt+0x15a>
				printfmt(putch, putdat, "error %d", err);
f0100f57:	50                   	push   %eax
f0100f58:	68 e1 1e 10 f0       	push   $0xf0101ee1
f0100f5d:	53                   	push   %ebx
f0100f5e:	56                   	push   %esi
f0100f5f:	e8 94 fe ff ff       	call   f0100df8 <printfmt>
f0100f64:	83 c4 10             	add    $0x10,%esp
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100f67:	8b 7d e4             	mov    -0x1c(%ebp),%edi
		case 'e':
			err = va_arg(ap, int);
			if (err < 0)
				err = -err;
			if (err >= MAXERROR || (p = error_string[err]) == NULL)
				printfmt(putch, putdat, "error %d", err);
f0100f6a:	e9 cc fe ff ff       	jmp    f0100e3b <vprintfmt+0x26>
			else
				printfmt(putch, putdat, "%s", p);
f0100f6f:	52                   	push   %edx
f0100f70:	68 ea 1e 10 f0       	push   $0xf0101eea
f0100f75:	53                   	push   %ebx
f0100f76:	56                   	push   %esi
f0100f77:	e8 7c fe ff ff       	call   f0100df8 <printfmt>
f0100f7c:	83 c4 10             	add    $0x10,%esp
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0100f7f:	8b 7d e4             	mov    -0x1c(%ebp),%edi
f0100f82:	e9 b4 fe ff ff       	jmp    f0100e3b <vprintfmt+0x26>
				printfmt(putch, putdat, "%s", p);
			break;

		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
f0100f87:	8b 45 14             	mov    0x14(%ebp),%eax
f0100f8a:	8d 50 04             	lea    0x4(%eax),%edx
f0100f8d:	89 55 14             	mov    %edx,0x14(%ebp)
f0100f90:	8b 38                	mov    (%eax),%edi
				p = "(null)";
f0100f92:	85 ff                	test   %edi,%edi
f0100f94:	b8 da 1e 10 f0       	mov    $0xf0101eda,%eax
f0100f99:	0f 44 f8             	cmove  %eax,%edi
			if (width > 0 && padc != '-')
f0100f9c:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
f0100fa0:	0f 8e 94 00 00 00    	jle    f010103a <vprintfmt+0x225>
f0100fa6:	80 7d d4 2d          	cmpb   $0x2d,-0x2c(%ebp)
f0100faa:	0f 84 98 00 00 00    	je     f0101048 <vprintfmt+0x233>
				for (width -= strnlen(p, precision); width > 0; width--)
f0100fb0:	83 ec 08             	sub    $0x8,%esp
f0100fb3:	ff 75 d0             	pushl  -0x30(%ebp)
f0100fb6:	57                   	push   %edi
f0100fb7:	e8 5f 03 00 00       	call   f010131b <strnlen>
f0100fbc:	8b 4d e0             	mov    -0x20(%ebp),%ecx
f0100fbf:	29 c1                	sub    %eax,%ecx
f0100fc1:	89 4d cc             	mov    %ecx,-0x34(%ebp)
f0100fc4:	83 c4 10             	add    $0x10,%esp
					putch(padc, putdat);
f0100fc7:	0f be 45 d4          	movsbl -0x2c(%ebp),%eax
f0100fcb:	89 45 e0             	mov    %eax,-0x20(%ebp)
f0100fce:	89 7d d4             	mov    %edi,-0x2c(%ebp)
f0100fd1:	89 cf                	mov    %ecx,%edi
		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
f0100fd3:	eb 0f                	jmp    f0100fe4 <vprintfmt+0x1cf>
					putch(padc, putdat);
f0100fd5:	83 ec 08             	sub    $0x8,%esp
f0100fd8:	53                   	push   %ebx
f0100fd9:	ff 75 e0             	pushl  -0x20(%ebp)
f0100fdc:	ff d6                	call   *%esi
		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
f0100fde:	83 ef 01             	sub    $0x1,%edi
f0100fe1:	83 c4 10             	add    $0x10,%esp
f0100fe4:	85 ff                	test   %edi,%edi
f0100fe6:	7f ed                	jg     f0100fd5 <vprintfmt+0x1c0>
f0100fe8:	8b 7d d4             	mov    -0x2c(%ebp),%edi
f0100feb:	8b 4d cc             	mov    -0x34(%ebp),%ecx
f0100fee:	85 c9                	test   %ecx,%ecx
f0100ff0:	b8 00 00 00 00       	mov    $0x0,%eax
f0100ff5:	0f 49 c1             	cmovns %ecx,%eax
f0100ff8:	29 c1                	sub    %eax,%ecx
f0100ffa:	89 75 08             	mov    %esi,0x8(%ebp)
f0100ffd:	8b 75 d0             	mov    -0x30(%ebp),%esi
f0101000:	89 5d 0c             	mov    %ebx,0xc(%ebp)
f0101003:	89 cb                	mov    %ecx,%ebx
f0101005:	eb 4d                	jmp    f0101054 <vprintfmt+0x23f>
					putch(padc, putdat);
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
				if (altflag && (ch < ' ' || ch > '~'))
f0101007:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
f010100b:	74 1b                	je     f0101028 <vprintfmt+0x213>
f010100d:	0f be c0             	movsbl %al,%eax
f0101010:	83 e8 20             	sub    $0x20,%eax
f0101013:	83 f8 5e             	cmp    $0x5e,%eax
f0101016:	76 10                	jbe    f0101028 <vprintfmt+0x213>
					putch('?', putdat);
f0101018:	83 ec 08             	sub    $0x8,%esp
f010101b:	ff 75 0c             	pushl  0xc(%ebp)
f010101e:	6a 3f                	push   $0x3f
f0101020:	ff 55 08             	call   *0x8(%ebp)
f0101023:	83 c4 10             	add    $0x10,%esp
f0101026:	eb 0d                	jmp    f0101035 <vprintfmt+0x220>
				else
					putch(ch, putdat);
f0101028:	83 ec 08             	sub    $0x8,%esp
f010102b:	ff 75 0c             	pushl  0xc(%ebp)
f010102e:	52                   	push   %edx
f010102f:	ff 55 08             	call   *0x8(%ebp)
f0101032:	83 c4 10             	add    $0x10,%esp
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
					putch(padc, putdat);
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
f0101035:	83 eb 01             	sub    $0x1,%ebx
f0101038:	eb 1a                	jmp    f0101054 <vprintfmt+0x23f>
f010103a:	89 75 08             	mov    %esi,0x8(%ebp)
f010103d:	8b 75 d0             	mov    -0x30(%ebp),%esi
f0101040:	89 5d 0c             	mov    %ebx,0xc(%ebp)
f0101043:	8b 5d e0             	mov    -0x20(%ebp),%ebx
f0101046:	eb 0c                	jmp    f0101054 <vprintfmt+0x23f>
f0101048:	89 75 08             	mov    %esi,0x8(%ebp)
f010104b:	8b 75 d0             	mov    -0x30(%ebp),%esi
f010104e:	89 5d 0c             	mov    %ebx,0xc(%ebp)
f0101051:	8b 5d e0             	mov    -0x20(%ebp),%ebx
f0101054:	83 c7 01             	add    $0x1,%edi
f0101057:	0f b6 47 ff          	movzbl -0x1(%edi),%eax
f010105b:	0f be d0             	movsbl %al,%edx
f010105e:	85 d2                	test   %edx,%edx
f0101060:	74 23                	je     f0101085 <vprintfmt+0x270>
f0101062:	85 f6                	test   %esi,%esi
f0101064:	78 a1                	js     f0101007 <vprintfmt+0x1f2>
f0101066:	83 ee 01             	sub    $0x1,%esi
f0101069:	79 9c                	jns    f0101007 <vprintfmt+0x1f2>
f010106b:	89 df                	mov    %ebx,%edi
f010106d:	8b 75 08             	mov    0x8(%ebp),%esi
f0101070:	8b 5d 0c             	mov    0xc(%ebp),%ebx
f0101073:	eb 18                	jmp    f010108d <vprintfmt+0x278>
				if (altflag && (ch < ' ' || ch > '~'))
					putch('?', putdat);
				else
					putch(ch, putdat);
			for (; width > 0; width--)
				putch(' ', putdat);
f0101075:	83 ec 08             	sub    $0x8,%esp
f0101078:	53                   	push   %ebx
f0101079:	6a 20                	push   $0x20
f010107b:	ff d6                	call   *%esi
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
				if (altflag && (ch < ' ' || ch > '~'))
					putch('?', putdat);
				else
					putch(ch, putdat);
			for (; width > 0; width--)
f010107d:	83 ef 01             	sub    $0x1,%edi
f0101080:	83 c4 10             	add    $0x10,%esp
f0101083:	eb 08                	jmp    f010108d <vprintfmt+0x278>
f0101085:	89 df                	mov    %ebx,%edi
f0101087:	8b 75 08             	mov    0x8(%ebp),%esi
f010108a:	8b 5d 0c             	mov    0xc(%ebp),%ebx
f010108d:	85 ff                	test   %edi,%edi
f010108f:	7f e4                	jg     f0101075 <vprintfmt+0x260>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0101091:	8b 7d e4             	mov    -0x1c(%ebp),%edi
f0101094:	e9 a2 fd ff ff       	jmp    f0100e3b <vprintfmt+0x26>
// Same as getuint but signed - can't use getuint
// because of sign extension
static long long
getint(va_list *ap, int lflag)
{
	if (lflag >= 2)
f0101099:	83 fa 01             	cmp    $0x1,%edx
f010109c:	7e 16                	jle    f01010b4 <vprintfmt+0x29f>
		return va_arg(*ap, long long);
f010109e:	8b 45 14             	mov    0x14(%ebp),%eax
f01010a1:	8d 50 08             	lea    0x8(%eax),%edx
f01010a4:	89 55 14             	mov    %edx,0x14(%ebp)
f01010a7:	8b 50 04             	mov    0x4(%eax),%edx
f01010aa:	8b 00                	mov    (%eax),%eax
f01010ac:	89 45 d8             	mov    %eax,-0x28(%ebp)
f01010af:	89 55 dc             	mov    %edx,-0x24(%ebp)
f01010b2:	eb 32                	jmp    f01010e6 <vprintfmt+0x2d1>
	else if (lflag)
f01010b4:	85 d2                	test   %edx,%edx
f01010b6:	74 18                	je     f01010d0 <vprintfmt+0x2bb>
		return va_arg(*ap, long);
f01010b8:	8b 45 14             	mov    0x14(%ebp),%eax
f01010bb:	8d 50 04             	lea    0x4(%eax),%edx
f01010be:	89 55 14             	mov    %edx,0x14(%ebp)
f01010c1:	8b 00                	mov    (%eax),%eax
f01010c3:	89 45 d8             	mov    %eax,-0x28(%ebp)
f01010c6:	89 c1                	mov    %eax,%ecx
f01010c8:	c1 f9 1f             	sar    $0x1f,%ecx
f01010cb:	89 4d dc             	mov    %ecx,-0x24(%ebp)
f01010ce:	eb 16                	jmp    f01010e6 <vprintfmt+0x2d1>
	else
		return va_arg(*ap, int);
f01010d0:	8b 45 14             	mov    0x14(%ebp),%eax
f01010d3:	8d 50 04             	lea    0x4(%eax),%edx
f01010d6:	89 55 14             	mov    %edx,0x14(%ebp)
f01010d9:	8b 00                	mov    (%eax),%eax
f01010db:	89 45 d8             	mov    %eax,-0x28(%ebp)
f01010de:	89 c1                	mov    %eax,%ecx
f01010e0:	c1 f9 1f             	sar    $0x1f,%ecx
f01010e3:	89 4d dc             	mov    %ecx,-0x24(%ebp)
				putch(' ', putdat);
			break;

		// (signed) decimal
		case 'd':
			num = getint(&ap, lflag);
f01010e6:	8b 45 d8             	mov    -0x28(%ebp),%eax
f01010e9:	8b 55 dc             	mov    -0x24(%ebp),%edx
			if ((long long) num < 0) {
				putch('-', putdat);
				num = -(long long) num;
			}
			base = 10;
f01010ec:	b9 0a 00 00 00       	mov    $0xa,%ecx
			break;

		// (signed) decimal
		case 'd':
			num = getint(&ap, lflag);
			if ((long long) num < 0) {
f01010f1:	83 7d dc 00          	cmpl   $0x0,-0x24(%ebp)
f01010f5:	79 74                	jns    f010116b <vprintfmt+0x356>
				putch('-', putdat);
f01010f7:	83 ec 08             	sub    $0x8,%esp
f01010fa:	53                   	push   %ebx
f01010fb:	6a 2d                	push   $0x2d
f01010fd:	ff d6                	call   *%esi
				num = -(long long) num;
f01010ff:	8b 45 d8             	mov    -0x28(%ebp),%eax
f0101102:	8b 55 dc             	mov    -0x24(%ebp),%edx
f0101105:	f7 d8                	neg    %eax
f0101107:	83 d2 00             	adc    $0x0,%edx
f010110a:	f7 da                	neg    %edx
f010110c:	83 c4 10             	add    $0x10,%esp
			}
			base = 10;
f010110f:	b9 0a 00 00 00       	mov    $0xa,%ecx
f0101114:	eb 55                	jmp    f010116b <vprintfmt+0x356>
			goto number;

		// unsigned decimal
		case 'u':
			num = getuint(&ap, lflag);
f0101116:	8d 45 14             	lea    0x14(%ebp),%eax
f0101119:	e8 83 fc ff ff       	call   f0100da1 <getuint>
			base = 10;
f010111e:	b9 0a 00 00 00       	mov    $0xa,%ecx
			goto number;
f0101123:	eb 46                	jmp    f010116b <vprintfmt+0x356>

		// (unsigned) octal
		case 'o':
			// Replace this with your code.
			num = getuint(&ap, lflag);
f0101125:	8d 45 14             	lea    0x14(%ebp),%eax
f0101128:	e8 74 fc ff ff       	call   f0100da1 <getuint>
			base = 8;
f010112d:	b9 08 00 00 00       	mov    $0x8,%ecx
			goto number;
f0101132:	eb 37                	jmp    f010116b <vprintfmt+0x356>

		// pointer
		case 'p':
			putch('0', putdat);
f0101134:	83 ec 08             	sub    $0x8,%esp
f0101137:	53                   	push   %ebx
f0101138:	6a 30                	push   $0x30
f010113a:	ff d6                	call   *%esi
			putch('x', putdat);
f010113c:	83 c4 08             	add    $0x8,%esp
f010113f:	53                   	push   %ebx
f0101140:	6a 78                	push   $0x78
f0101142:	ff d6                	call   *%esi
			num = (unsigned long long)
				(uintptr_t) va_arg(ap, void *);
f0101144:	8b 45 14             	mov    0x14(%ebp),%eax
f0101147:	8d 50 04             	lea    0x4(%eax),%edx
f010114a:	89 55 14             	mov    %edx,0x14(%ebp)

		// pointer
		case 'p':
			putch('0', putdat);
			putch('x', putdat);
			num = (unsigned long long)
f010114d:	8b 00                	mov    (%eax),%eax
f010114f:	ba 00 00 00 00       	mov    $0x0,%edx
				(uintptr_t) va_arg(ap, void *);
			base = 16;
			goto number;
f0101154:	83 c4 10             	add    $0x10,%esp
		case 'p':
			putch('0', putdat);
			putch('x', putdat);
			num = (unsigned long long)
				(uintptr_t) va_arg(ap, void *);
			base = 16;
f0101157:	b9 10 00 00 00       	mov    $0x10,%ecx
			goto number;
f010115c:	eb 0d                	jmp    f010116b <vprintfmt+0x356>

		// (unsigned) hexadecimal
		case 'x':
			num = getuint(&ap, lflag);
f010115e:	8d 45 14             	lea    0x14(%ebp),%eax
f0101161:	e8 3b fc ff ff       	call   f0100da1 <getuint>
			base = 16;
f0101166:	b9 10 00 00 00       	mov    $0x10,%ecx
		number:
			printnum(putch, putdat, num, base, width, padc);
f010116b:	83 ec 0c             	sub    $0xc,%esp
f010116e:	0f be 7d d4          	movsbl -0x2c(%ebp),%edi
f0101172:	57                   	push   %edi
f0101173:	ff 75 e0             	pushl  -0x20(%ebp)
f0101176:	51                   	push   %ecx
f0101177:	52                   	push   %edx
f0101178:	50                   	push   %eax
f0101179:	89 da                	mov    %ebx,%edx
f010117b:	89 f0                	mov    %esi,%eax
f010117d:	e8 70 fb ff ff       	call   f0100cf2 <printnum>
			break;
f0101182:	83 c4 20             	add    $0x20,%esp
f0101185:	8b 7d e4             	mov    -0x1c(%ebp),%edi
f0101188:	e9 ae fc ff ff       	jmp    f0100e3b <vprintfmt+0x26>

		// escaped '%' character
		case '%':
			putch(ch, putdat);
f010118d:	83 ec 08             	sub    $0x8,%esp
f0101190:	53                   	push   %ebx
f0101191:	51                   	push   %ecx
f0101192:	ff d6                	call   *%esi
			break;
f0101194:	83 c4 10             	add    $0x10,%esp
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
f0101197:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			break;

		// escaped '%' character
		case '%':
			putch(ch, putdat);
			break;
f010119a:	e9 9c fc ff ff       	jmp    f0100e3b <vprintfmt+0x26>

		// unrecognized escape sequence - just print it literally
		default:
			putch('%', putdat);
f010119f:	83 ec 08             	sub    $0x8,%esp
f01011a2:	53                   	push   %ebx
f01011a3:	6a 25                	push   $0x25
f01011a5:	ff d6                	call   *%esi
			for (fmt--; fmt[-1] != '%'; fmt--)
f01011a7:	83 c4 10             	add    $0x10,%esp
f01011aa:	eb 03                	jmp    f01011af <vprintfmt+0x39a>
f01011ac:	83 ef 01             	sub    $0x1,%edi
f01011af:	80 7f ff 25          	cmpb   $0x25,-0x1(%edi)
f01011b3:	75 f7                	jne    f01011ac <vprintfmt+0x397>
f01011b5:	e9 81 fc ff ff       	jmp    f0100e3b <vprintfmt+0x26>
				/* do nothing */;
			break;
		}
	}
}
f01011ba:	8d 65 f4             	lea    -0xc(%ebp),%esp
f01011bd:	5b                   	pop    %ebx
f01011be:	5e                   	pop    %esi
f01011bf:	5f                   	pop    %edi
f01011c0:	5d                   	pop    %ebp
f01011c1:	c3                   	ret    

f01011c2 <vsnprintf>:
		*b->buf++ = ch;
}

int
vsnprintf(char *buf, int n, const char *fmt, va_list ap)
{
f01011c2:	55                   	push   %ebp
f01011c3:	89 e5                	mov    %esp,%ebp
f01011c5:	83 ec 18             	sub    $0x18,%esp
f01011c8:	8b 45 08             	mov    0x8(%ebp),%eax
f01011cb:	8b 55 0c             	mov    0xc(%ebp),%edx
	struct sprintbuf b = {buf, buf+n-1, 0};
f01011ce:	89 45 ec             	mov    %eax,-0x14(%ebp)
f01011d1:	8d 4c 10 ff          	lea    -0x1(%eax,%edx,1),%ecx
f01011d5:	89 4d f0             	mov    %ecx,-0x10(%ebp)
f01011d8:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)

	if (buf == NULL || n < 1)
f01011df:	85 c0                	test   %eax,%eax
f01011e1:	74 26                	je     f0101209 <vsnprintf+0x47>
f01011e3:	85 d2                	test   %edx,%edx
f01011e5:	7e 22                	jle    f0101209 <vsnprintf+0x47>
		return -E_INVAL;

	// print the string to the buffer
	vprintfmt((void*)sprintputch, &b, fmt, ap);
f01011e7:	ff 75 14             	pushl  0x14(%ebp)
f01011ea:	ff 75 10             	pushl  0x10(%ebp)
f01011ed:	8d 45 ec             	lea    -0x14(%ebp),%eax
f01011f0:	50                   	push   %eax
f01011f1:	68 db 0d 10 f0       	push   $0xf0100ddb
f01011f6:	e8 1a fc ff ff       	call   f0100e15 <vprintfmt>

	// null terminate the buffer
	*b.buf = '\0';
f01011fb:	8b 45 ec             	mov    -0x14(%ebp),%eax
f01011fe:	c6 00 00             	movb   $0x0,(%eax)

	return b.cnt;
f0101201:	8b 45 f4             	mov    -0xc(%ebp),%eax
f0101204:	83 c4 10             	add    $0x10,%esp
f0101207:	eb 05                	jmp    f010120e <vsnprintf+0x4c>
vsnprintf(char *buf, int n, const char *fmt, va_list ap)
{
	struct sprintbuf b = {buf, buf+n-1, 0};

	if (buf == NULL || n < 1)
		return -E_INVAL;
f0101209:	b8 fd ff ff ff       	mov    $0xfffffffd,%eax

	// null terminate the buffer
	*b.buf = '\0';

	return b.cnt;
}
f010120e:	c9                   	leave  
f010120f:	c3                   	ret    

f0101210 <snprintf>:

int
snprintf(char *buf, int n, const char *fmt, ...)
{
f0101210:	55                   	push   %ebp
f0101211:	89 e5                	mov    %esp,%ebp
f0101213:	83 ec 08             	sub    $0x8,%esp
	va_list ap;
	int rc;

	va_start(ap, fmt);
f0101216:	8d 45 14             	lea    0x14(%ebp),%eax
	rc = vsnprintf(buf, n, fmt, ap);
f0101219:	50                   	push   %eax
f010121a:	ff 75 10             	pushl  0x10(%ebp)
f010121d:	ff 75 0c             	pushl  0xc(%ebp)
f0101220:	ff 75 08             	pushl  0x8(%ebp)
f0101223:	e8 9a ff ff ff       	call   f01011c2 <vsnprintf>
	va_end(ap);

	return rc;
}
f0101228:	c9                   	leave  
f0101229:	c3                   	ret    

f010122a <readline>:
#define BUFLEN 1024
static char buf[BUFLEN];

char *
readline(const char *prompt)
{
f010122a:	55                   	push   %ebp
f010122b:	89 e5                	mov    %esp,%ebp
f010122d:	57                   	push   %edi
f010122e:	56                   	push   %esi
f010122f:	53                   	push   %ebx
f0101230:	83 ec 0c             	sub    $0xc,%esp
f0101233:	8b 45 08             	mov    0x8(%ebp),%eax
	int i, c, echoing;

	if (prompt != NULL)
f0101236:	85 c0                	test   %eax,%eax
f0101238:	74 11                	je     f010124b <readline+0x21>
		cprintf("%s", prompt);
f010123a:	83 ec 08             	sub    $0x8,%esp
f010123d:	50                   	push   %eax
f010123e:	68 ea 1e 10 f0       	push   $0xf0101eea
f0101243:	e8 90 f7 ff ff       	call   f01009d8 <cprintf>
f0101248:	83 c4 10             	add    $0x10,%esp

	i = 0;
	echoing = iscons(0);
f010124b:	83 ec 0c             	sub    $0xc,%esp
f010124e:	6a 00                	push   $0x0
f0101250:	e8 27 f4 ff ff       	call   f010067c <iscons>
f0101255:	89 c7                	mov    %eax,%edi
f0101257:	83 c4 10             	add    $0x10,%esp
	int i, c, echoing;

	if (prompt != NULL)
		cprintf("%s", prompt);

	i = 0;
f010125a:	be 00 00 00 00       	mov    $0x0,%esi
	echoing = iscons(0);
	while (1) {
		c = getchar();
f010125f:	e8 07 f4 ff ff       	call   f010066b <getchar>
f0101264:	89 c3                	mov    %eax,%ebx
		if (c < 0) {
f0101266:	85 c0                	test   %eax,%eax
f0101268:	79 18                	jns    f0101282 <readline+0x58>
			cprintf("read error: %e\n", c);
f010126a:	83 ec 08             	sub    $0x8,%esp
f010126d:	50                   	push   %eax
f010126e:	68 cc 20 10 f0       	push   $0xf01020cc
f0101273:	e8 60 f7 ff ff       	call   f01009d8 <cprintf>
			return NULL;
f0101278:	83 c4 10             	add    $0x10,%esp
f010127b:	b8 00 00 00 00       	mov    $0x0,%eax
f0101280:	eb 79                	jmp    f01012fb <readline+0xd1>
		} else if ((c == '\b' || c == '\x7f') && i > 0) {
f0101282:	83 f8 08             	cmp    $0x8,%eax
f0101285:	0f 94 c2             	sete   %dl
f0101288:	83 f8 7f             	cmp    $0x7f,%eax
f010128b:	0f 94 c0             	sete   %al
f010128e:	08 c2                	or     %al,%dl
f0101290:	74 1a                	je     f01012ac <readline+0x82>
f0101292:	85 f6                	test   %esi,%esi
f0101294:	7e 16                	jle    f01012ac <readline+0x82>
			if (echoing)
f0101296:	85 ff                	test   %edi,%edi
f0101298:	74 0d                	je     f01012a7 <readline+0x7d>
				cputchar('\b');
f010129a:	83 ec 0c             	sub    $0xc,%esp
f010129d:	6a 08                	push   $0x8
f010129f:	e8 b7 f3 ff ff       	call   f010065b <cputchar>
f01012a4:	83 c4 10             	add    $0x10,%esp
			i--;
f01012a7:	83 ee 01             	sub    $0x1,%esi
f01012aa:	eb b3                	jmp    f010125f <readline+0x35>
		} else if (c >= ' ' && i < BUFLEN-1) {
f01012ac:	83 fb 1f             	cmp    $0x1f,%ebx
f01012af:	7e 23                	jle    f01012d4 <readline+0xaa>
f01012b1:	81 fe fe 03 00 00    	cmp    $0x3fe,%esi
f01012b7:	7f 1b                	jg     f01012d4 <readline+0xaa>
			if (echoing)
f01012b9:	85 ff                	test   %edi,%edi
f01012bb:	74 0c                	je     f01012c9 <readline+0x9f>
				cputchar(c);
f01012bd:	83 ec 0c             	sub    $0xc,%esp
f01012c0:	53                   	push   %ebx
f01012c1:	e8 95 f3 ff ff       	call   f010065b <cputchar>
f01012c6:	83 c4 10             	add    $0x10,%esp
			buf[i++] = c;
f01012c9:	88 9e 40 25 11 f0    	mov    %bl,-0xfeedac0(%esi)
f01012cf:	8d 76 01             	lea    0x1(%esi),%esi
f01012d2:	eb 8b                	jmp    f010125f <readline+0x35>
		} else if (c == '\n' || c == '\r') {
f01012d4:	83 fb 0a             	cmp    $0xa,%ebx
f01012d7:	74 05                	je     f01012de <readline+0xb4>
f01012d9:	83 fb 0d             	cmp    $0xd,%ebx
f01012dc:	75 81                	jne    f010125f <readline+0x35>
			if (echoing)
f01012de:	85 ff                	test   %edi,%edi
f01012e0:	74 0d                	je     f01012ef <readline+0xc5>
				cputchar('\n');
f01012e2:	83 ec 0c             	sub    $0xc,%esp
f01012e5:	6a 0a                	push   $0xa
f01012e7:	e8 6f f3 ff ff       	call   f010065b <cputchar>
f01012ec:	83 c4 10             	add    $0x10,%esp
			buf[i] = 0;
f01012ef:	c6 86 40 25 11 f0 00 	movb   $0x0,-0xfeedac0(%esi)
			return buf;
f01012f6:	b8 40 25 11 f0       	mov    $0xf0112540,%eax
		}
	}
}
f01012fb:	8d 65 f4             	lea    -0xc(%ebp),%esp
f01012fe:	5b                   	pop    %ebx
f01012ff:	5e                   	pop    %esi
f0101300:	5f                   	pop    %edi
f0101301:	5d                   	pop    %ebp
f0101302:	c3                   	ret    

f0101303 <strlen>:
// Primespipe runs 3x faster this way.
#define ASM 1

int
strlen(const char *s)
{
f0101303:	55                   	push   %ebp
f0101304:	89 e5                	mov    %esp,%ebp
f0101306:	8b 55 08             	mov    0x8(%ebp),%edx
	int n;

	for (n = 0; *s != '\0'; s++)
f0101309:	b8 00 00 00 00       	mov    $0x0,%eax
f010130e:	eb 03                	jmp    f0101313 <strlen+0x10>
		n++;
f0101310:	83 c0 01             	add    $0x1,%eax
int
strlen(const char *s)
{
	int n;

	for (n = 0; *s != '\0'; s++)
f0101313:	80 3c 02 00          	cmpb   $0x0,(%edx,%eax,1)
f0101317:	75 f7                	jne    f0101310 <strlen+0xd>
		n++;
	return n;
}
f0101319:	5d                   	pop    %ebp
f010131a:	c3                   	ret    

f010131b <strnlen>:

int
strnlen(const char *s, size_t size)
{
f010131b:	55                   	push   %ebp
f010131c:	89 e5                	mov    %esp,%ebp
f010131e:	8b 4d 08             	mov    0x8(%ebp),%ecx
f0101321:	8b 45 0c             	mov    0xc(%ebp),%eax
	int n;

	for (n = 0; size > 0 && *s != '\0'; s++, size--)
f0101324:	ba 00 00 00 00       	mov    $0x0,%edx
f0101329:	eb 03                	jmp    f010132e <strnlen+0x13>
		n++;
f010132b:	83 c2 01             	add    $0x1,%edx
int
strnlen(const char *s, size_t size)
{
	int n;

	for (n = 0; size > 0 && *s != '\0'; s++, size--)
f010132e:	39 c2                	cmp    %eax,%edx
f0101330:	74 08                	je     f010133a <strnlen+0x1f>
f0101332:	80 3c 11 00          	cmpb   $0x0,(%ecx,%edx,1)
f0101336:	75 f3                	jne    f010132b <strnlen+0x10>
f0101338:	89 d0                	mov    %edx,%eax
		n++;
	return n;
}
f010133a:	5d                   	pop    %ebp
f010133b:	c3                   	ret    

f010133c <strcpy>:

char *
strcpy(char *dst, const char *src)
{
f010133c:	55                   	push   %ebp
f010133d:	89 e5                	mov    %esp,%ebp
f010133f:	53                   	push   %ebx
f0101340:	8b 45 08             	mov    0x8(%ebp),%eax
f0101343:	8b 4d 0c             	mov    0xc(%ebp),%ecx
	char *ret;

	ret = dst;
	while ((*dst++ = *src++) != '\0')
f0101346:	89 c2                	mov    %eax,%edx
f0101348:	83 c2 01             	add    $0x1,%edx
f010134b:	83 c1 01             	add    $0x1,%ecx
f010134e:	0f b6 59 ff          	movzbl -0x1(%ecx),%ebx
f0101352:	88 5a ff             	mov    %bl,-0x1(%edx)
f0101355:	84 db                	test   %bl,%bl
f0101357:	75 ef                	jne    f0101348 <strcpy+0xc>
		/* do nothing */;
	return ret;
}
f0101359:	5b                   	pop    %ebx
f010135a:	5d                   	pop    %ebp
f010135b:	c3                   	ret    

f010135c <strcat>:

char *
strcat(char *dst, const char *src)
{
f010135c:	55                   	push   %ebp
f010135d:	89 e5                	mov    %esp,%ebp
f010135f:	53                   	push   %ebx
f0101360:	8b 5d 08             	mov    0x8(%ebp),%ebx
	int len = strlen(dst);
f0101363:	53                   	push   %ebx
f0101364:	e8 9a ff ff ff       	call   f0101303 <strlen>
f0101369:	83 c4 04             	add    $0x4,%esp
	strcpy(dst + len, src);
f010136c:	ff 75 0c             	pushl  0xc(%ebp)
f010136f:	01 d8                	add    %ebx,%eax
f0101371:	50                   	push   %eax
f0101372:	e8 c5 ff ff ff       	call   f010133c <strcpy>
	return dst;
}
f0101377:	89 d8                	mov    %ebx,%eax
f0101379:	8b 5d fc             	mov    -0x4(%ebp),%ebx
f010137c:	c9                   	leave  
f010137d:	c3                   	ret    

f010137e <strncpy>:

char *
strncpy(char *dst, const char *src, size_t size) {
f010137e:	55                   	push   %ebp
f010137f:	89 e5                	mov    %esp,%ebp
f0101381:	56                   	push   %esi
f0101382:	53                   	push   %ebx
f0101383:	8b 75 08             	mov    0x8(%ebp),%esi
f0101386:	8b 4d 0c             	mov    0xc(%ebp),%ecx
f0101389:	89 f3                	mov    %esi,%ebx
f010138b:	03 5d 10             	add    0x10(%ebp),%ebx
	size_t i;
	char *ret;

	ret = dst;
	for (i = 0; i < size; i++) {
f010138e:	89 f2                	mov    %esi,%edx
f0101390:	eb 0f                	jmp    f01013a1 <strncpy+0x23>
		*dst++ = *src;
f0101392:	83 c2 01             	add    $0x1,%edx
f0101395:	0f b6 01             	movzbl (%ecx),%eax
f0101398:	88 42 ff             	mov    %al,-0x1(%edx)
		// If strlen(src) < size, null-pad 'dst' out to 'size' chars
		if (*src != '\0')
			src++;
f010139b:	80 39 01             	cmpb   $0x1,(%ecx)
f010139e:	83 d9 ff             	sbb    $0xffffffff,%ecx
strncpy(char *dst, const char *src, size_t size) {
	size_t i;
	char *ret;

	ret = dst;
	for (i = 0; i < size; i++) {
f01013a1:	39 da                	cmp    %ebx,%edx
f01013a3:	75 ed                	jne    f0101392 <strncpy+0x14>
		// If strlen(src) < size, null-pad 'dst' out to 'size' chars
		if (*src != '\0')
			src++;
	}
	return ret;
}
f01013a5:	89 f0                	mov    %esi,%eax
f01013a7:	5b                   	pop    %ebx
f01013a8:	5e                   	pop    %esi
f01013a9:	5d                   	pop    %ebp
f01013aa:	c3                   	ret    

f01013ab <strlcpy>:

size_t
strlcpy(char *dst, const char *src, size_t size)
{
f01013ab:	55                   	push   %ebp
f01013ac:	89 e5                	mov    %esp,%ebp
f01013ae:	56                   	push   %esi
f01013af:	53                   	push   %ebx
f01013b0:	8b 75 08             	mov    0x8(%ebp),%esi
f01013b3:	8b 4d 0c             	mov    0xc(%ebp),%ecx
f01013b6:	8b 55 10             	mov    0x10(%ebp),%edx
f01013b9:	89 f0                	mov    %esi,%eax
	char *dst_in;

	dst_in = dst;
	if (size > 0) {
f01013bb:	85 d2                	test   %edx,%edx
f01013bd:	74 21                	je     f01013e0 <strlcpy+0x35>
f01013bf:	8d 44 16 ff          	lea    -0x1(%esi,%edx,1),%eax
f01013c3:	89 f2                	mov    %esi,%edx
f01013c5:	eb 09                	jmp    f01013d0 <strlcpy+0x25>
		while (--size > 0 && *src != '\0')
			*dst++ = *src++;
f01013c7:	83 c2 01             	add    $0x1,%edx
f01013ca:	83 c1 01             	add    $0x1,%ecx
f01013cd:	88 5a ff             	mov    %bl,-0x1(%edx)
{
	char *dst_in;

	dst_in = dst;
	if (size > 0) {
		while (--size > 0 && *src != '\0')
f01013d0:	39 c2                	cmp    %eax,%edx
f01013d2:	74 09                	je     f01013dd <strlcpy+0x32>
f01013d4:	0f b6 19             	movzbl (%ecx),%ebx
f01013d7:	84 db                	test   %bl,%bl
f01013d9:	75 ec                	jne    f01013c7 <strlcpy+0x1c>
f01013db:	89 d0                	mov    %edx,%eax
			*dst++ = *src++;
		*dst = '\0';
f01013dd:	c6 00 00             	movb   $0x0,(%eax)
	}
	return dst - dst_in;
f01013e0:	29 f0                	sub    %esi,%eax
}
f01013e2:	5b                   	pop    %ebx
f01013e3:	5e                   	pop    %esi
f01013e4:	5d                   	pop    %ebp
f01013e5:	c3                   	ret    

f01013e6 <strcmp>:

int
strcmp(const char *p, const char *q)
{
f01013e6:	55                   	push   %ebp
f01013e7:	89 e5                	mov    %esp,%ebp
f01013e9:	8b 4d 08             	mov    0x8(%ebp),%ecx
f01013ec:	8b 55 0c             	mov    0xc(%ebp),%edx
	while (*p && *p == *q)
f01013ef:	eb 06                	jmp    f01013f7 <strcmp+0x11>
		p++, q++;
f01013f1:	83 c1 01             	add    $0x1,%ecx
f01013f4:	83 c2 01             	add    $0x1,%edx
}

int
strcmp(const char *p, const char *q)
{
	while (*p && *p == *q)
f01013f7:	0f b6 01             	movzbl (%ecx),%eax
f01013fa:	84 c0                	test   %al,%al
f01013fc:	74 04                	je     f0101402 <strcmp+0x1c>
f01013fe:	3a 02                	cmp    (%edx),%al
f0101400:	74 ef                	je     f01013f1 <strcmp+0xb>
		p++, q++;
	return (int) ((unsigned char) *p - (unsigned char) *q);
f0101402:	0f b6 c0             	movzbl %al,%eax
f0101405:	0f b6 12             	movzbl (%edx),%edx
f0101408:	29 d0                	sub    %edx,%eax
}
f010140a:	5d                   	pop    %ebp
f010140b:	c3                   	ret    

f010140c <strncmp>:

int
strncmp(const char *p, const char *q, size_t n)
{
f010140c:	55                   	push   %ebp
f010140d:	89 e5                	mov    %esp,%ebp
f010140f:	53                   	push   %ebx
f0101410:	8b 45 08             	mov    0x8(%ebp),%eax
f0101413:	8b 55 0c             	mov    0xc(%ebp),%edx
f0101416:	89 c3                	mov    %eax,%ebx
f0101418:	03 5d 10             	add    0x10(%ebp),%ebx
	while (n > 0 && *p && *p == *q)
f010141b:	eb 06                	jmp    f0101423 <strncmp+0x17>
		n--, p++, q++;
f010141d:	83 c0 01             	add    $0x1,%eax
f0101420:	83 c2 01             	add    $0x1,%edx
}

int
strncmp(const char *p, const char *q, size_t n)
{
	while (n > 0 && *p && *p == *q)
f0101423:	39 d8                	cmp    %ebx,%eax
f0101425:	74 15                	je     f010143c <strncmp+0x30>
f0101427:	0f b6 08             	movzbl (%eax),%ecx
f010142a:	84 c9                	test   %cl,%cl
f010142c:	74 04                	je     f0101432 <strncmp+0x26>
f010142e:	3a 0a                	cmp    (%edx),%cl
f0101430:	74 eb                	je     f010141d <strncmp+0x11>
		n--, p++, q++;
	if (n == 0)
		return 0;
	else
		return (int) ((unsigned char) *p - (unsigned char) *q);
f0101432:	0f b6 00             	movzbl (%eax),%eax
f0101435:	0f b6 12             	movzbl (%edx),%edx
f0101438:	29 d0                	sub    %edx,%eax
f010143a:	eb 05                	jmp    f0101441 <strncmp+0x35>
strncmp(const char *p, const char *q, size_t n)
{
	while (n > 0 && *p && *p == *q)
		n--, p++, q++;
	if (n == 0)
		return 0;
f010143c:	b8 00 00 00 00       	mov    $0x0,%eax
	else
		return (int) ((unsigned char) *p - (unsigned char) *q);
}
f0101441:	5b                   	pop    %ebx
f0101442:	5d                   	pop    %ebp
f0101443:	c3                   	ret    

f0101444 <strchr>:

// Return a pointer to the first occurrence of 'c' in 's',
// or a null pointer if the string has no 'c'.
char *
strchr(const char *s, char c)
{
f0101444:	55                   	push   %ebp
f0101445:	89 e5                	mov    %esp,%ebp
f0101447:	8b 45 08             	mov    0x8(%ebp),%eax
f010144a:	0f b6 4d 0c          	movzbl 0xc(%ebp),%ecx
	for (; *s; s++)
f010144e:	eb 07                	jmp    f0101457 <strchr+0x13>
		if (*s == c)
f0101450:	38 ca                	cmp    %cl,%dl
f0101452:	74 0f                	je     f0101463 <strchr+0x1f>
// Return a pointer to the first occurrence of 'c' in 's',
// or a null pointer if the string has no 'c'.
char *
strchr(const char *s, char c)
{
	for (; *s; s++)
f0101454:	83 c0 01             	add    $0x1,%eax
f0101457:	0f b6 10             	movzbl (%eax),%edx
f010145a:	84 d2                	test   %dl,%dl
f010145c:	75 f2                	jne    f0101450 <strchr+0xc>
		if (*s == c)
			return (char *) s;
	return 0;
f010145e:	b8 00 00 00 00       	mov    $0x0,%eax
}
f0101463:	5d                   	pop    %ebp
f0101464:	c3                   	ret    

f0101465 <strfind>:

// Return a pointer to the first occurrence of 'c' in 's',
// or a pointer to the string-ending null character if the string has no 'c'.
char *
strfind(const char *s, char c)
{
f0101465:	55                   	push   %ebp
f0101466:	89 e5                	mov    %esp,%ebp
f0101468:	8b 45 08             	mov    0x8(%ebp),%eax
f010146b:	0f b6 4d 0c          	movzbl 0xc(%ebp),%ecx
	for (; *s; s++)
f010146f:	eb 03                	jmp    f0101474 <strfind+0xf>
f0101471:	83 c0 01             	add    $0x1,%eax
f0101474:	0f b6 10             	movzbl (%eax),%edx
		if (*s == c)
f0101477:	38 ca                	cmp    %cl,%dl
f0101479:	74 04                	je     f010147f <strfind+0x1a>
f010147b:	84 d2                	test   %dl,%dl
f010147d:	75 f2                	jne    f0101471 <strfind+0xc>
			break;
	return (char *) s;
}
f010147f:	5d                   	pop    %ebp
f0101480:	c3                   	ret    

f0101481 <memset>:

#if ASM
void *
memset(void *v, int c, size_t n)
{
f0101481:	55                   	push   %ebp
f0101482:	89 e5                	mov    %esp,%ebp
f0101484:	57                   	push   %edi
f0101485:	56                   	push   %esi
f0101486:	53                   	push   %ebx
f0101487:	8b 7d 08             	mov    0x8(%ebp),%edi
f010148a:	8b 4d 10             	mov    0x10(%ebp),%ecx
	char *p;

	if (n == 0)
f010148d:	85 c9                	test   %ecx,%ecx
f010148f:	74 36                	je     f01014c7 <memset+0x46>
		return v;
	if ((int)v%4 == 0 && n%4 == 0) {
f0101491:	f7 c7 03 00 00 00    	test   $0x3,%edi
f0101497:	75 28                	jne    f01014c1 <memset+0x40>
f0101499:	f6 c1 03             	test   $0x3,%cl
f010149c:	75 23                	jne    f01014c1 <memset+0x40>
		c &= 0xFF;
f010149e:	0f b6 55 0c          	movzbl 0xc(%ebp),%edx
		c = (c<<24)|(c<<16)|(c<<8)|c;
f01014a2:	89 d3                	mov    %edx,%ebx
f01014a4:	c1 e3 08             	shl    $0x8,%ebx
f01014a7:	89 d6                	mov    %edx,%esi
f01014a9:	c1 e6 18             	shl    $0x18,%esi
f01014ac:	89 d0                	mov    %edx,%eax
f01014ae:	c1 e0 10             	shl    $0x10,%eax
f01014b1:	09 f0                	or     %esi,%eax
f01014b3:	09 c2                	or     %eax,%edx
		asm volatile("cld; rep stosl\n"
f01014b5:	89 d8                	mov    %ebx,%eax
f01014b7:	09 d0                	or     %edx,%eax
f01014b9:	c1 e9 02             	shr    $0x2,%ecx
f01014bc:	fc                   	cld    
f01014bd:	f3 ab                	rep stos %eax,%es:(%edi)
f01014bf:	eb 06                	jmp    f01014c7 <memset+0x46>
			:: "D" (v), "a" (c), "c" (n/4)
			: "cc", "memory");
	} else
		asm volatile("cld; rep stosb\n"
f01014c1:	8b 45 0c             	mov    0xc(%ebp),%eax
f01014c4:	fc                   	cld    
f01014c5:	f3 aa                	rep stos %al,%es:(%edi)
			:: "D" (v), "a" (c), "c" (n)
			: "cc", "memory");
	return v;
}
f01014c7:	89 f8                	mov    %edi,%eax
f01014c9:	5b                   	pop    %ebx
f01014ca:	5e                   	pop    %esi
f01014cb:	5f                   	pop    %edi
f01014cc:	5d                   	pop    %ebp
f01014cd:	c3                   	ret    

f01014ce <memmove>:

void *
memmove(void *dst, const void *src, size_t n)
{
f01014ce:	55                   	push   %ebp
f01014cf:	89 e5                	mov    %esp,%ebp
f01014d1:	57                   	push   %edi
f01014d2:	56                   	push   %esi
f01014d3:	8b 45 08             	mov    0x8(%ebp),%eax
f01014d6:	8b 75 0c             	mov    0xc(%ebp),%esi
f01014d9:	8b 4d 10             	mov    0x10(%ebp),%ecx
	const char *s;
	char *d;

	s = src;
	d = dst;
	if (s < d && s + n > d) {
f01014dc:	39 c6                	cmp    %eax,%esi
f01014de:	73 35                	jae    f0101515 <memmove+0x47>
f01014e0:	8d 14 0e             	lea    (%esi,%ecx,1),%edx
f01014e3:	39 d0                	cmp    %edx,%eax
f01014e5:	73 2e                	jae    f0101515 <memmove+0x47>
		s += n;
		d += n;
f01014e7:	8d 3c 08             	lea    (%eax,%ecx,1),%edi
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
f01014ea:	89 d6                	mov    %edx,%esi
f01014ec:	09 fe                	or     %edi,%esi
f01014ee:	f7 c6 03 00 00 00    	test   $0x3,%esi
f01014f4:	75 13                	jne    f0101509 <memmove+0x3b>
f01014f6:	f6 c1 03             	test   $0x3,%cl
f01014f9:	75 0e                	jne    f0101509 <memmove+0x3b>
			asm volatile("std; rep movsl\n"
f01014fb:	83 ef 04             	sub    $0x4,%edi
f01014fe:	8d 72 fc             	lea    -0x4(%edx),%esi
f0101501:	c1 e9 02             	shr    $0x2,%ecx
f0101504:	fd                   	std    
f0101505:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
f0101507:	eb 09                	jmp    f0101512 <memmove+0x44>
				:: "D" (d-4), "S" (s-4), "c" (n/4) : "cc", "memory");
		else
			asm volatile("std; rep movsb\n"
f0101509:	83 ef 01             	sub    $0x1,%edi
f010150c:	8d 72 ff             	lea    -0x1(%edx),%esi
f010150f:	fd                   	std    
f0101510:	f3 a4                	rep movsb %ds:(%esi),%es:(%edi)
				:: "D" (d-1), "S" (s-1), "c" (n) : "cc", "memory");
		// Some versions of GCC rely on DF being clear
		asm volatile("cld" ::: "cc");
f0101512:	fc                   	cld    
f0101513:	eb 1d                	jmp    f0101532 <memmove+0x64>
	} else {
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
f0101515:	89 f2                	mov    %esi,%edx
f0101517:	09 c2                	or     %eax,%edx
f0101519:	f6 c2 03             	test   $0x3,%dl
f010151c:	75 0f                	jne    f010152d <memmove+0x5f>
f010151e:	f6 c1 03             	test   $0x3,%cl
f0101521:	75 0a                	jne    f010152d <memmove+0x5f>
			asm volatile("cld; rep movsl\n"
f0101523:	c1 e9 02             	shr    $0x2,%ecx
f0101526:	89 c7                	mov    %eax,%edi
f0101528:	fc                   	cld    
f0101529:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
f010152b:	eb 05                	jmp    f0101532 <memmove+0x64>
				:: "D" (d), "S" (s), "c" (n/4) : "cc", "memory");
		else
			asm volatile("cld; rep movsb\n"
f010152d:	89 c7                	mov    %eax,%edi
f010152f:	fc                   	cld    
f0101530:	f3 a4                	rep movsb %ds:(%esi),%es:(%edi)
				:: "D" (d), "S" (s), "c" (n) : "cc", "memory");
	}
	return dst;
}
f0101532:	5e                   	pop    %esi
f0101533:	5f                   	pop    %edi
f0101534:	5d                   	pop    %ebp
f0101535:	c3                   	ret    

f0101536 <memcpy>:
}
#endif

void *
memcpy(void *dst, const void *src, size_t n)
{
f0101536:	55                   	push   %ebp
f0101537:	89 e5                	mov    %esp,%ebp
	return memmove(dst, src, n);
f0101539:	ff 75 10             	pushl  0x10(%ebp)
f010153c:	ff 75 0c             	pushl  0xc(%ebp)
f010153f:	ff 75 08             	pushl  0x8(%ebp)
f0101542:	e8 87 ff ff ff       	call   f01014ce <memmove>
}
f0101547:	c9                   	leave  
f0101548:	c3                   	ret    

f0101549 <memcmp>:

int
memcmp(const void *v1, const void *v2, size_t n)
{
f0101549:	55                   	push   %ebp
f010154a:	89 e5                	mov    %esp,%ebp
f010154c:	56                   	push   %esi
f010154d:	53                   	push   %ebx
f010154e:	8b 45 08             	mov    0x8(%ebp),%eax
f0101551:	8b 55 0c             	mov    0xc(%ebp),%edx
f0101554:	89 c6                	mov    %eax,%esi
f0101556:	03 75 10             	add    0x10(%ebp),%esi
	const uint8_t *s1 = (const uint8_t *) v1;
	const uint8_t *s2 = (const uint8_t *) v2;

	while (n-- > 0) {
f0101559:	eb 1a                	jmp    f0101575 <memcmp+0x2c>
		if (*s1 != *s2)
f010155b:	0f b6 08             	movzbl (%eax),%ecx
f010155e:	0f b6 1a             	movzbl (%edx),%ebx
f0101561:	38 d9                	cmp    %bl,%cl
f0101563:	74 0a                	je     f010156f <memcmp+0x26>
			return (int) *s1 - (int) *s2;
f0101565:	0f b6 c1             	movzbl %cl,%eax
f0101568:	0f b6 db             	movzbl %bl,%ebx
f010156b:	29 d8                	sub    %ebx,%eax
f010156d:	eb 0f                	jmp    f010157e <memcmp+0x35>
		s1++, s2++;
f010156f:	83 c0 01             	add    $0x1,%eax
f0101572:	83 c2 01             	add    $0x1,%edx
memcmp(const void *v1, const void *v2, size_t n)
{
	const uint8_t *s1 = (const uint8_t *) v1;
	const uint8_t *s2 = (const uint8_t *) v2;

	while (n-- > 0) {
f0101575:	39 f0                	cmp    %esi,%eax
f0101577:	75 e2                	jne    f010155b <memcmp+0x12>
		if (*s1 != *s2)
			return (int) *s1 - (int) *s2;
		s1++, s2++;
	}

	return 0;
f0101579:	b8 00 00 00 00       	mov    $0x0,%eax
}
f010157e:	5b                   	pop    %ebx
f010157f:	5e                   	pop    %esi
f0101580:	5d                   	pop    %ebp
f0101581:	c3                   	ret    

f0101582 <memfind>:

void *
memfind(const void *s, int c, size_t n)
{
f0101582:	55                   	push   %ebp
f0101583:	89 e5                	mov    %esp,%ebp
f0101585:	53                   	push   %ebx
f0101586:	8b 45 08             	mov    0x8(%ebp),%eax
	const void *ends = (const char *) s + n;
f0101589:	89 c1                	mov    %eax,%ecx
f010158b:	03 4d 10             	add    0x10(%ebp),%ecx
	for (; s < ends; s++)
		if (*(const unsigned char *) s == (unsigned char) c)
f010158e:	0f b6 5d 0c          	movzbl 0xc(%ebp),%ebx

void *
memfind(const void *s, int c, size_t n)
{
	const void *ends = (const char *) s + n;
	for (; s < ends; s++)
f0101592:	eb 0a                	jmp    f010159e <memfind+0x1c>
		if (*(const unsigned char *) s == (unsigned char) c)
f0101594:	0f b6 10             	movzbl (%eax),%edx
f0101597:	39 da                	cmp    %ebx,%edx
f0101599:	74 07                	je     f01015a2 <memfind+0x20>

void *
memfind(const void *s, int c, size_t n)
{
	const void *ends = (const char *) s + n;
	for (; s < ends; s++)
f010159b:	83 c0 01             	add    $0x1,%eax
f010159e:	39 c8                	cmp    %ecx,%eax
f01015a0:	72 f2                	jb     f0101594 <memfind+0x12>
		if (*(const unsigned char *) s == (unsigned char) c)
			break;
	return (void *) s;
}
f01015a2:	5b                   	pop    %ebx
f01015a3:	5d                   	pop    %ebp
f01015a4:	c3                   	ret    

f01015a5 <strtol>:

long
strtol(const char *s, char **endptr, int base)
{
f01015a5:	55                   	push   %ebp
f01015a6:	89 e5                	mov    %esp,%ebp
f01015a8:	57                   	push   %edi
f01015a9:	56                   	push   %esi
f01015aa:	53                   	push   %ebx
f01015ab:	8b 4d 08             	mov    0x8(%ebp),%ecx
f01015ae:	8b 5d 10             	mov    0x10(%ebp),%ebx
	int neg = 0;
	long val = 0;

	// gobble initial whitespace
	while (*s == ' ' || *s == '\t')
f01015b1:	eb 03                	jmp    f01015b6 <strtol+0x11>
		s++;
f01015b3:	83 c1 01             	add    $0x1,%ecx
{
	int neg = 0;
	long val = 0;

	// gobble initial whitespace
	while (*s == ' ' || *s == '\t')
f01015b6:	0f b6 01             	movzbl (%ecx),%eax
f01015b9:	3c 20                	cmp    $0x20,%al
f01015bb:	74 f6                	je     f01015b3 <strtol+0xe>
f01015bd:	3c 09                	cmp    $0x9,%al
f01015bf:	74 f2                	je     f01015b3 <strtol+0xe>
		s++;

	// plus/minus sign
	if (*s == '+')
f01015c1:	3c 2b                	cmp    $0x2b,%al
f01015c3:	75 0a                	jne    f01015cf <strtol+0x2a>
		s++;
f01015c5:	83 c1 01             	add    $0x1,%ecx
}

long
strtol(const char *s, char **endptr, int base)
{
	int neg = 0;
f01015c8:	bf 00 00 00 00       	mov    $0x0,%edi
f01015cd:	eb 11                	jmp    f01015e0 <strtol+0x3b>
f01015cf:	bf 00 00 00 00       	mov    $0x0,%edi
		s++;

	// plus/minus sign
	if (*s == '+')
		s++;
	else if (*s == '-')
f01015d4:	3c 2d                	cmp    $0x2d,%al
f01015d6:	75 08                	jne    f01015e0 <strtol+0x3b>
		s++, neg = 1;
f01015d8:	83 c1 01             	add    $0x1,%ecx
f01015db:	bf 01 00 00 00       	mov    $0x1,%edi

	// hex or octal base prefix
	if ((base == 0 || base == 16) && (s[0] == '0' && s[1] == 'x'))
f01015e0:	f7 c3 ef ff ff ff    	test   $0xffffffef,%ebx
f01015e6:	75 15                	jne    f01015fd <strtol+0x58>
f01015e8:	80 39 30             	cmpb   $0x30,(%ecx)
f01015eb:	75 10                	jne    f01015fd <strtol+0x58>
f01015ed:	80 79 01 78          	cmpb   $0x78,0x1(%ecx)
f01015f1:	75 7c                	jne    f010166f <strtol+0xca>
		s += 2, base = 16;
f01015f3:	83 c1 02             	add    $0x2,%ecx
f01015f6:	bb 10 00 00 00       	mov    $0x10,%ebx
f01015fb:	eb 16                	jmp    f0101613 <strtol+0x6e>
	else if (base == 0 && s[0] == '0')
f01015fd:	85 db                	test   %ebx,%ebx
f01015ff:	75 12                	jne    f0101613 <strtol+0x6e>
		s++, base = 8;
	else if (base == 0)
		base = 10;
f0101601:	bb 0a 00 00 00       	mov    $0xa,%ebx
		s++, neg = 1;

	// hex or octal base prefix
	if ((base == 0 || base == 16) && (s[0] == '0' && s[1] == 'x'))
		s += 2, base = 16;
	else if (base == 0 && s[0] == '0')
f0101606:	80 39 30             	cmpb   $0x30,(%ecx)
f0101609:	75 08                	jne    f0101613 <strtol+0x6e>
		s++, base = 8;
f010160b:	83 c1 01             	add    $0x1,%ecx
f010160e:	bb 08 00 00 00       	mov    $0x8,%ebx
	else if (base == 0)
		base = 10;
f0101613:	b8 00 00 00 00       	mov    $0x0,%eax
f0101618:	89 5d 10             	mov    %ebx,0x10(%ebp)

	// digits
	while (1) {
		int dig;

		if (*s >= '0' && *s <= '9')
f010161b:	0f b6 11             	movzbl (%ecx),%edx
f010161e:	8d 72 d0             	lea    -0x30(%edx),%esi
f0101621:	89 f3                	mov    %esi,%ebx
f0101623:	80 fb 09             	cmp    $0x9,%bl
f0101626:	77 08                	ja     f0101630 <strtol+0x8b>
			dig = *s - '0';
f0101628:	0f be d2             	movsbl %dl,%edx
f010162b:	83 ea 30             	sub    $0x30,%edx
f010162e:	eb 22                	jmp    f0101652 <strtol+0xad>
		else if (*s >= 'a' && *s <= 'z')
f0101630:	8d 72 9f             	lea    -0x61(%edx),%esi
f0101633:	89 f3                	mov    %esi,%ebx
f0101635:	80 fb 19             	cmp    $0x19,%bl
f0101638:	77 08                	ja     f0101642 <strtol+0x9d>
			dig = *s - 'a' + 10;
f010163a:	0f be d2             	movsbl %dl,%edx
f010163d:	83 ea 57             	sub    $0x57,%edx
f0101640:	eb 10                	jmp    f0101652 <strtol+0xad>
		else if (*s >= 'A' && *s <= 'Z')
f0101642:	8d 72 bf             	lea    -0x41(%edx),%esi
f0101645:	89 f3                	mov    %esi,%ebx
f0101647:	80 fb 19             	cmp    $0x19,%bl
f010164a:	77 16                	ja     f0101662 <strtol+0xbd>
			dig = *s - 'A' + 10;
f010164c:	0f be d2             	movsbl %dl,%edx
f010164f:	83 ea 37             	sub    $0x37,%edx
		else
			break;
		if (dig >= base)
f0101652:	3b 55 10             	cmp    0x10(%ebp),%edx
f0101655:	7d 0b                	jge    f0101662 <strtol+0xbd>
			break;
		s++, val = (val * base) + dig;
f0101657:	83 c1 01             	add    $0x1,%ecx
f010165a:	0f af 45 10          	imul   0x10(%ebp),%eax
f010165e:	01 d0                	add    %edx,%eax
		// we don't properly detect overflow!
	}
f0101660:	eb b9                	jmp    f010161b <strtol+0x76>

	if (endptr)
f0101662:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
f0101666:	74 0d                	je     f0101675 <strtol+0xd0>
		*endptr = (char *) s;
f0101668:	8b 75 0c             	mov    0xc(%ebp),%esi
f010166b:	89 0e                	mov    %ecx,(%esi)
f010166d:	eb 06                	jmp    f0101675 <strtol+0xd0>
		s++, neg = 1;

	// hex or octal base prefix
	if ((base == 0 || base == 16) && (s[0] == '0' && s[1] == 'x'))
		s += 2, base = 16;
	else if (base == 0 && s[0] == '0')
f010166f:	85 db                	test   %ebx,%ebx
f0101671:	74 98                	je     f010160b <strtol+0x66>
f0101673:	eb 9e                	jmp    f0101613 <strtol+0x6e>
		// we don't properly detect overflow!
	}

	if (endptr)
		*endptr = (char *) s;
	return (neg ? -val : val);
f0101675:	89 c2                	mov    %eax,%edx
f0101677:	f7 da                	neg    %edx
f0101679:	85 ff                	test   %edi,%edi
f010167b:	0f 45 c2             	cmovne %edx,%eax
}
f010167e:	5b                   	pop    %ebx
f010167f:	5e                   	pop    %esi
f0101680:	5f                   	pop    %edi
f0101681:	5d                   	pop    %ebp
f0101682:	c3                   	ret    
f0101683:	66 90                	xchg   %ax,%ax
f0101685:	66 90                	xchg   %ax,%ax
f0101687:	66 90                	xchg   %ax,%ax
f0101689:	66 90                	xchg   %ax,%ax
f010168b:	66 90                	xchg   %ax,%ax
f010168d:	66 90                	xchg   %ax,%ax
f010168f:	90                   	nop

f0101690 <__udivdi3>:
f0101690:	55                   	push   %ebp
f0101691:	57                   	push   %edi
f0101692:	56                   	push   %esi
f0101693:	53                   	push   %ebx
f0101694:	83 ec 1c             	sub    $0x1c,%esp
f0101697:	8b 74 24 3c          	mov    0x3c(%esp),%esi
f010169b:	8b 5c 24 30          	mov    0x30(%esp),%ebx
f010169f:	8b 4c 24 34          	mov    0x34(%esp),%ecx
f01016a3:	8b 7c 24 38          	mov    0x38(%esp),%edi
f01016a7:	85 f6                	test   %esi,%esi
f01016a9:	89 5c 24 08          	mov    %ebx,0x8(%esp)
f01016ad:	89 ca                	mov    %ecx,%edx
f01016af:	89 f8                	mov    %edi,%eax
f01016b1:	75 3d                	jne    f01016f0 <__udivdi3+0x60>
f01016b3:	39 cf                	cmp    %ecx,%edi
f01016b5:	0f 87 c5 00 00 00    	ja     f0101780 <__udivdi3+0xf0>
f01016bb:	85 ff                	test   %edi,%edi
f01016bd:	89 fd                	mov    %edi,%ebp
f01016bf:	75 0b                	jne    f01016cc <__udivdi3+0x3c>
f01016c1:	b8 01 00 00 00       	mov    $0x1,%eax
f01016c6:	31 d2                	xor    %edx,%edx
f01016c8:	f7 f7                	div    %edi
f01016ca:	89 c5                	mov    %eax,%ebp
f01016cc:	89 c8                	mov    %ecx,%eax
f01016ce:	31 d2                	xor    %edx,%edx
f01016d0:	f7 f5                	div    %ebp
f01016d2:	89 c1                	mov    %eax,%ecx
f01016d4:	89 d8                	mov    %ebx,%eax
f01016d6:	89 cf                	mov    %ecx,%edi
f01016d8:	f7 f5                	div    %ebp
f01016da:	89 c3                	mov    %eax,%ebx
f01016dc:	89 d8                	mov    %ebx,%eax
f01016de:	89 fa                	mov    %edi,%edx
f01016e0:	83 c4 1c             	add    $0x1c,%esp
f01016e3:	5b                   	pop    %ebx
f01016e4:	5e                   	pop    %esi
f01016e5:	5f                   	pop    %edi
f01016e6:	5d                   	pop    %ebp
f01016e7:	c3                   	ret    
f01016e8:	90                   	nop
f01016e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
f01016f0:	39 ce                	cmp    %ecx,%esi
f01016f2:	77 74                	ja     f0101768 <__udivdi3+0xd8>
f01016f4:	0f bd fe             	bsr    %esi,%edi
f01016f7:	83 f7 1f             	xor    $0x1f,%edi
f01016fa:	0f 84 98 00 00 00    	je     f0101798 <__udivdi3+0x108>
f0101700:	bb 20 00 00 00       	mov    $0x20,%ebx
f0101705:	89 f9                	mov    %edi,%ecx
f0101707:	89 c5                	mov    %eax,%ebp
f0101709:	29 fb                	sub    %edi,%ebx
f010170b:	d3 e6                	shl    %cl,%esi
f010170d:	89 d9                	mov    %ebx,%ecx
f010170f:	d3 ed                	shr    %cl,%ebp
f0101711:	89 f9                	mov    %edi,%ecx
f0101713:	d3 e0                	shl    %cl,%eax
f0101715:	09 ee                	or     %ebp,%esi
f0101717:	89 d9                	mov    %ebx,%ecx
f0101719:	89 44 24 0c          	mov    %eax,0xc(%esp)
f010171d:	89 d5                	mov    %edx,%ebp
f010171f:	8b 44 24 08          	mov    0x8(%esp),%eax
f0101723:	d3 ed                	shr    %cl,%ebp
f0101725:	89 f9                	mov    %edi,%ecx
f0101727:	d3 e2                	shl    %cl,%edx
f0101729:	89 d9                	mov    %ebx,%ecx
f010172b:	d3 e8                	shr    %cl,%eax
f010172d:	09 c2                	or     %eax,%edx
f010172f:	89 d0                	mov    %edx,%eax
f0101731:	89 ea                	mov    %ebp,%edx
f0101733:	f7 f6                	div    %esi
f0101735:	89 d5                	mov    %edx,%ebp
f0101737:	89 c3                	mov    %eax,%ebx
f0101739:	f7 64 24 0c          	mull   0xc(%esp)
f010173d:	39 d5                	cmp    %edx,%ebp
f010173f:	72 10                	jb     f0101751 <__udivdi3+0xc1>
f0101741:	8b 74 24 08          	mov    0x8(%esp),%esi
f0101745:	89 f9                	mov    %edi,%ecx
f0101747:	d3 e6                	shl    %cl,%esi
f0101749:	39 c6                	cmp    %eax,%esi
f010174b:	73 07                	jae    f0101754 <__udivdi3+0xc4>
f010174d:	39 d5                	cmp    %edx,%ebp
f010174f:	75 03                	jne    f0101754 <__udivdi3+0xc4>
f0101751:	83 eb 01             	sub    $0x1,%ebx
f0101754:	31 ff                	xor    %edi,%edi
f0101756:	89 d8                	mov    %ebx,%eax
f0101758:	89 fa                	mov    %edi,%edx
f010175a:	83 c4 1c             	add    $0x1c,%esp
f010175d:	5b                   	pop    %ebx
f010175e:	5e                   	pop    %esi
f010175f:	5f                   	pop    %edi
f0101760:	5d                   	pop    %ebp
f0101761:	c3                   	ret    
f0101762:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
f0101768:	31 ff                	xor    %edi,%edi
f010176a:	31 db                	xor    %ebx,%ebx
f010176c:	89 d8                	mov    %ebx,%eax
f010176e:	89 fa                	mov    %edi,%edx
f0101770:	83 c4 1c             	add    $0x1c,%esp
f0101773:	5b                   	pop    %ebx
f0101774:	5e                   	pop    %esi
f0101775:	5f                   	pop    %edi
f0101776:	5d                   	pop    %ebp
f0101777:	c3                   	ret    
f0101778:	90                   	nop
f0101779:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
f0101780:	89 d8                	mov    %ebx,%eax
f0101782:	f7 f7                	div    %edi
f0101784:	31 ff                	xor    %edi,%edi
f0101786:	89 c3                	mov    %eax,%ebx
f0101788:	89 d8                	mov    %ebx,%eax
f010178a:	89 fa                	mov    %edi,%edx
f010178c:	83 c4 1c             	add    $0x1c,%esp
f010178f:	5b                   	pop    %ebx
f0101790:	5e                   	pop    %esi
f0101791:	5f                   	pop    %edi
f0101792:	5d                   	pop    %ebp
f0101793:	c3                   	ret    
f0101794:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
f0101798:	39 ce                	cmp    %ecx,%esi
f010179a:	72 0c                	jb     f01017a8 <__udivdi3+0x118>
f010179c:	31 db                	xor    %ebx,%ebx
f010179e:	3b 44 24 08          	cmp    0x8(%esp),%eax
f01017a2:	0f 87 34 ff ff ff    	ja     f01016dc <__udivdi3+0x4c>
f01017a8:	bb 01 00 00 00       	mov    $0x1,%ebx
f01017ad:	e9 2a ff ff ff       	jmp    f01016dc <__udivdi3+0x4c>
f01017b2:	66 90                	xchg   %ax,%ax
f01017b4:	66 90                	xchg   %ax,%ax
f01017b6:	66 90                	xchg   %ax,%ax
f01017b8:	66 90                	xchg   %ax,%ax
f01017ba:	66 90                	xchg   %ax,%ax
f01017bc:	66 90                	xchg   %ax,%ax
f01017be:	66 90                	xchg   %ax,%ax

f01017c0 <__umoddi3>:
f01017c0:	55                   	push   %ebp
f01017c1:	57                   	push   %edi
f01017c2:	56                   	push   %esi
f01017c3:	53                   	push   %ebx
f01017c4:	83 ec 1c             	sub    $0x1c,%esp
f01017c7:	8b 54 24 3c          	mov    0x3c(%esp),%edx
f01017cb:	8b 4c 24 30          	mov    0x30(%esp),%ecx
f01017cf:	8b 74 24 34          	mov    0x34(%esp),%esi
f01017d3:	8b 7c 24 38          	mov    0x38(%esp),%edi
f01017d7:	85 d2                	test   %edx,%edx
f01017d9:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
f01017dd:	89 4c 24 08          	mov    %ecx,0x8(%esp)
f01017e1:	89 f3                	mov    %esi,%ebx
f01017e3:	89 3c 24             	mov    %edi,(%esp)
f01017e6:	89 74 24 04          	mov    %esi,0x4(%esp)
f01017ea:	75 1c                	jne    f0101808 <__umoddi3+0x48>
f01017ec:	39 f7                	cmp    %esi,%edi
f01017ee:	76 50                	jbe    f0101840 <__umoddi3+0x80>
f01017f0:	89 c8                	mov    %ecx,%eax
f01017f2:	89 f2                	mov    %esi,%edx
f01017f4:	f7 f7                	div    %edi
f01017f6:	89 d0                	mov    %edx,%eax
f01017f8:	31 d2                	xor    %edx,%edx
f01017fa:	83 c4 1c             	add    $0x1c,%esp
f01017fd:	5b                   	pop    %ebx
f01017fe:	5e                   	pop    %esi
f01017ff:	5f                   	pop    %edi
f0101800:	5d                   	pop    %ebp
f0101801:	c3                   	ret    
f0101802:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
f0101808:	39 f2                	cmp    %esi,%edx
f010180a:	89 d0                	mov    %edx,%eax
f010180c:	77 52                	ja     f0101860 <__umoddi3+0xa0>
f010180e:	0f bd ea             	bsr    %edx,%ebp
f0101811:	83 f5 1f             	xor    $0x1f,%ebp
f0101814:	75 5a                	jne    f0101870 <__umoddi3+0xb0>
f0101816:	3b 54 24 04          	cmp    0x4(%esp),%edx
f010181a:	0f 82 e0 00 00 00    	jb     f0101900 <__umoddi3+0x140>
f0101820:	39 0c 24             	cmp    %ecx,(%esp)
f0101823:	0f 86 d7 00 00 00    	jbe    f0101900 <__umoddi3+0x140>
f0101829:	8b 44 24 08          	mov    0x8(%esp),%eax
f010182d:	8b 54 24 04          	mov    0x4(%esp),%edx
f0101831:	83 c4 1c             	add    $0x1c,%esp
f0101834:	5b                   	pop    %ebx
f0101835:	5e                   	pop    %esi
f0101836:	5f                   	pop    %edi
f0101837:	5d                   	pop    %ebp
f0101838:	c3                   	ret    
f0101839:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
f0101840:	85 ff                	test   %edi,%edi
f0101842:	89 fd                	mov    %edi,%ebp
f0101844:	75 0b                	jne    f0101851 <__umoddi3+0x91>
f0101846:	b8 01 00 00 00       	mov    $0x1,%eax
f010184b:	31 d2                	xor    %edx,%edx
f010184d:	f7 f7                	div    %edi
f010184f:	89 c5                	mov    %eax,%ebp
f0101851:	89 f0                	mov    %esi,%eax
f0101853:	31 d2                	xor    %edx,%edx
f0101855:	f7 f5                	div    %ebp
f0101857:	89 c8                	mov    %ecx,%eax
f0101859:	f7 f5                	div    %ebp
f010185b:	89 d0                	mov    %edx,%eax
f010185d:	eb 99                	jmp    f01017f8 <__umoddi3+0x38>
f010185f:	90                   	nop
f0101860:	89 c8                	mov    %ecx,%eax
f0101862:	89 f2                	mov    %esi,%edx
f0101864:	83 c4 1c             	add    $0x1c,%esp
f0101867:	5b                   	pop    %ebx
f0101868:	5e                   	pop    %esi
f0101869:	5f                   	pop    %edi
f010186a:	5d                   	pop    %ebp
f010186b:	c3                   	ret    
f010186c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
f0101870:	8b 34 24             	mov    (%esp),%esi
f0101873:	bf 20 00 00 00       	mov    $0x20,%edi
f0101878:	89 e9                	mov    %ebp,%ecx
f010187a:	29 ef                	sub    %ebp,%edi
f010187c:	d3 e0                	shl    %cl,%eax
f010187e:	89 f9                	mov    %edi,%ecx
f0101880:	89 f2                	mov    %esi,%edx
f0101882:	d3 ea                	shr    %cl,%edx
f0101884:	89 e9                	mov    %ebp,%ecx
f0101886:	09 c2                	or     %eax,%edx
f0101888:	89 d8                	mov    %ebx,%eax
f010188a:	89 14 24             	mov    %edx,(%esp)
f010188d:	89 f2                	mov    %esi,%edx
f010188f:	d3 e2                	shl    %cl,%edx
f0101891:	89 f9                	mov    %edi,%ecx
f0101893:	89 54 24 04          	mov    %edx,0x4(%esp)
f0101897:	8b 54 24 0c          	mov    0xc(%esp),%edx
f010189b:	d3 e8                	shr    %cl,%eax
f010189d:	89 e9                	mov    %ebp,%ecx
f010189f:	89 c6                	mov    %eax,%esi
f01018a1:	d3 e3                	shl    %cl,%ebx
f01018a3:	89 f9                	mov    %edi,%ecx
f01018a5:	89 d0                	mov    %edx,%eax
f01018a7:	d3 e8                	shr    %cl,%eax
f01018a9:	89 e9                	mov    %ebp,%ecx
f01018ab:	09 d8                	or     %ebx,%eax
f01018ad:	89 d3                	mov    %edx,%ebx
f01018af:	89 f2                	mov    %esi,%edx
f01018b1:	f7 34 24             	divl   (%esp)
f01018b4:	89 d6                	mov    %edx,%esi
f01018b6:	d3 e3                	shl    %cl,%ebx
f01018b8:	f7 64 24 04          	mull   0x4(%esp)
f01018bc:	39 d6                	cmp    %edx,%esi
f01018be:	89 5c 24 08          	mov    %ebx,0x8(%esp)
f01018c2:	89 d1                	mov    %edx,%ecx
f01018c4:	89 c3                	mov    %eax,%ebx
f01018c6:	72 08                	jb     f01018d0 <__umoddi3+0x110>
f01018c8:	75 11                	jne    f01018db <__umoddi3+0x11b>
f01018ca:	39 44 24 08          	cmp    %eax,0x8(%esp)
f01018ce:	73 0b                	jae    f01018db <__umoddi3+0x11b>
f01018d0:	2b 44 24 04          	sub    0x4(%esp),%eax
f01018d4:	1b 14 24             	sbb    (%esp),%edx
f01018d7:	89 d1                	mov    %edx,%ecx
f01018d9:	89 c3                	mov    %eax,%ebx
f01018db:	8b 54 24 08          	mov    0x8(%esp),%edx
f01018df:	29 da                	sub    %ebx,%edx
f01018e1:	19 ce                	sbb    %ecx,%esi
f01018e3:	89 f9                	mov    %edi,%ecx
f01018e5:	89 f0                	mov    %esi,%eax
f01018e7:	d3 e0                	shl    %cl,%eax
f01018e9:	89 e9                	mov    %ebp,%ecx
f01018eb:	d3 ea                	shr    %cl,%edx
f01018ed:	89 e9                	mov    %ebp,%ecx
f01018ef:	d3 ee                	shr    %cl,%esi
f01018f1:	09 d0                	or     %edx,%eax
f01018f3:	89 f2                	mov    %esi,%edx
f01018f5:	83 c4 1c             	add    $0x1c,%esp
f01018f8:	5b                   	pop    %ebx
f01018f9:	5e                   	pop    %esi
f01018fa:	5f                   	pop    %edi
f01018fb:	5d                   	pop    %ebp
f01018fc:	c3                   	ret    
f01018fd:	8d 76 00             	lea    0x0(%esi),%esi
f0101900:	29 f9                	sub    %edi,%ecx
f0101902:	19 d6                	sbb    %edx,%esi
f0101904:	89 74 24 04          	mov    %esi,0x4(%esp)
f0101908:	89 4c 24 08          	mov    %ecx,0x8(%esp)
f010190c:	e9 18 ff ff ff       	jmp    f0101829 <__umoddi3+0x69>
